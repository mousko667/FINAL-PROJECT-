...
function KpiCard({ title, value, sub, icon, color }: { title: string; value: string | number; sub?: string; icon: React.ReactNode; color: string }) {
  return (
    <div className="bg-surface rounded-xl border border-hairline p-5 flex items-start gap-4">
    ...
    <PageRoleGuard allowedRoles={['ROLE_DAF', 'ROLE_ASSISTANT_COMPTABLE']}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-ink-faint mt-1">{t('reports.subtitle')}</p>
        </div>

        {/* Date range selector */}
        <div className="bg-surface rounded-xl border border-hairline p-5 flex flex-wrap items-end gap-4">
          <div>
          ...
          <p className="text-xs text-ink-faint self-center">{t('reports.dateRangeHint')}</p>
        </div>
        ...
        <Section title={t('reports.kpiTitle')}>
          {kpiLoading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard title={t('dashboard.totalInvoices')} value={kpi?.totalInvoices ?? '—'} icon={<TrendingUp className="w-5 h-5 text-primary" />} color="bg-primary/10" />
              <KpiCard title={t('dashboard.overdueInvoices')} value={kpi?.overdueCount ?? '—'} icon={<AlertTriangle className="w-5 h-5 text-crit" />} color="bg-crit/10" />
        ...
        <Section title={t('reports.budgetTitle')}>
          <p className="text-sm text-ink-faint mb-4">{t('reports.budgetDesc')}</p>
          {budgetLoading ? (
            <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
          ) : !budget?.lines?.length ? (
            <p className="text-sm text-center text-ink-faint py-4">{t('reports.noData')}</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-ink-faint hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                    <th className="px-3 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.budgetDept')}</th>
                    <th className="px-3 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.budgetColBudget')}</th>
                    <th className="px-3 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.budgetColActual')}</th>
                    <th className="px-3 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.budgetColVariance')}</th>
                    <th className="px-3 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.budgetColUtil')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-hairline">
                  {budget.lines.map(l => {
                    ...
                    return (
                      <tr key={l.departmentCode} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                        ...
                        <td className={`px-3 py-2 text-right font-medium ${l.variance == null ? 'text-gray-300' : over ? 'text-crit' : 'text-pos'}`}>
                ...
                <tfoot>
                  <tr className="border-t-2 font-semibold text-ink hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
        ...
        <Section title={t('reports.agingTitle')}>
          <p className="text-sm text-ink-faint mb-4">{t('reports.agingDesc')}</p>
          {agingLoading ? (
            <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
            ...
            <div className="space-y-3">
              {aging.buckets.map(b => (
                <div key={b.label} className="flex items-center gap-4">
                  ...
                  <div className="text-xs text-ink-faint w-20 text-right shrink-0">
        ...
        <Section title={t('reports.bottleneckTitle')} defaultOpen={false}>
          <p className="text-sm text-ink-faint mb-4">{t('reports.bottleneckDesc')}</p>
          {bnLoading ? (
            <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
          ) : !bottlenecks?.length ? (
            <p className="text-sm text-center text-ink-faint py-4">No SLA breaches detected — all approvals are within the 3-day SLA.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-ink-faint hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                  <th className="pb-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.approver')}</th>
                  <th className="pb-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.invoiceCount')}</th>
                  <th className="pb-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reports.avgDays')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-hairline">
                {bottlenecks.map(b => (
                  <tr key={b.approverUsername} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                    ...
                    <td className="py-2.5 text-right">
                      <span className={`font-medium ${b.averageDays > 5 ? 'text-crit' : b.averageDays > 3 ? 'text-amber-600' : 'text-pos'}`}>
        ...
        <Section title={t('reports.supplierPerformance.title')} defaultOpen={false}>
          <p className="text-sm text-ink-faint mb-4">{t('reports.supplierPerformance.desc')}</p>
          <div className="mb-4 max-w-md">
            <label htmlFor="perf-supplier" className="block text-xs font-medium text-ink-faint uppercase tracking-wide mb-1">
          ...
          {!perfSupplierId ? (
            <p className="text-sm text-center text-ink-faint py-4">{t('reports.supplierPerformance.prompt')}</p>
          ) : perfLoading ? (
            <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
          ) : !supplierPerf ? (
            <p className="text-sm text-center text-ink-faint py-4">{t('reports.noData')}</p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard
                ...
                icon={<Target className="w-5 h-5 text-pos" />} color="bg-pos/10"
              />
              <KpiCard
              ...
              <KpiCard
                ...
                icon={<CalendarClock className="w-5 h-5 text-primary" />} color="bg-primary/10"
              />
              <KpiCard
                ...
                icon={<FileStack className="w-5 h-5 text-primary" />} color="bg-primary/10"
              />
            </div>
          )}
        </Section>
        ...
        <Section title={t('reports.cashFlowTitle')} defaultOpen={false}>
          <p className="text-sm text-ink-faint mb-4">{t('reports.cashFlowDesc')}</p>
          {cfLoading ? (
            <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
        ...
        <Section title="Data Exports">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Excel export */}
            <div className="flex items-start gap-4 p-4 rounded-xl border bg-ground">
              <div className="p-2.5 bg-pos/10 rounded-xl shrink-0">
                <FileSpreadsheet className="w-5 h-5 text-pos" />
              </div>
              <div className="flex-1">
                <h3 className="font-semibold text-ink text-sm">{t('reports.exportExcel')}</h3>
                <p className="text-xs text-ink-faint mt-0.5">{t('reports.exportExcelDesc')}</p>
              </div>
              <button
            ...
            <div className="flex items-start gap-4 p-4 rounded-xl border bg-ground">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-ink-faint mt-0.5">{t('reports.exportPdfComplianceDesc')}</p>
                {(!fromDate || !toDate) && (
