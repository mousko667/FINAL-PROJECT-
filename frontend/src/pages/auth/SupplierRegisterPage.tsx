...
export default function SupplierRegisterPage() {
  ...
  const barColors = ['bg-red-400', 'bg-yellow-400', 'bg-blue-400', 'bg-pos/100']
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-surface rounded-2xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-ink-faint mb-6">{t('supplier.register.successMessage', 'Check your email to verify your account before logging in.')}</p>
      ...
      <div className="bg-surface rounded-2xl shadow-xl p-8 w-full max-w-2xl">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-ink-faint">{t('supplier.register.subtitle', 'Create your supplier account to submit invoices')}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <>
                  <div className="mt-2 grid grid-cols-4 gap-1" aria-label="Password strength">
                    {[0, 1, 2, 3].map((index) => (
                      <div
                        key={index}
                        className={`h-1.5 rounded-full transition-colors ${index < passwordStrength ? activeBarColor : 'bg-surface-hover'}`}
                      />
                    ))}
                  </div>
          ...
          {serverError && <p className="text-sm text-red-500 bg-crit/10 px-4 py-2 rounded-lg">{serverError}</p>}
