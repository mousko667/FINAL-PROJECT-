...
export default function SupplierProfilePage() {
  ...
  return (
    <form onSubmit={handleSubmit(values => mutation.mutate(values))} className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t('supplier.profile.title')}</h1>
        <p className="text-sm text-gray-500 mt-1">{t('supplier.invoice.submit.subtitle')}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="block text-sm font-medium text-gray-700">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Tax ID cannot be changed.</p>
          </label>
          <label className="block text-sm font-medium text-gray-700">
      ...
      <div className="bg-white rounded-xl border p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">{t('supplier.profile.bankDetails')}</h2>
        </div>
        <p className="text-sm text-gray-500">{t('supplier.profile.bankDetailsHint')}</p>
        <label className="block text-sm font-medium text-gray-700">
          <textarea
