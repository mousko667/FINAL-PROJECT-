...
function MfaSection({ profile }: { profile: StaffProfile }) {
  ...
  return (
    <div className="bg-surface border-hairline rounded-xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        {isMfaActive ? (
          <span className="flex items-center gap-1.5 text-xs font-semibold text-pos bg-pos/10 px-2.5 py-1 rounded-full">
            <ShieldCheck className="w-3.5 h-3.5" />
            {t('mfa.active', 'Activé')}
          </span>
        ) : (
          <span className="flex items-center gap-1.5 text-xs font-semibold text-crit bg-crit/10 px-2.5 py-1 rounded-full">
      ...
      {confirmSuccess && (
        <div className="flex items-center gap-2 text-sm text-pos bg-pos/10 border border-green-200 rounded-lg px-3 py-2.5">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-6 items-start">
            ...
            <div className="flex-shrink-0">
              <div
                className="border rounded-lg p-2 bg-surface inline-block"
                role="img"
      ...
      {isMfaActive && !setupData && (
        <button
          ...
          onClick={() => setupMutation.mutate()}
          disabled={setupMutation.isPending}
          className="flex items-center gap-2 text-sm text-ink-faint hover:text-ink-soft transition-colors"
        >
          <QrCode className="w-4 h-4" />
  ...
  return (
    <form onSubmit={handleSubmit((values) => mutation.mutate(values))} className="max-w-4xl space-y-5 page-enter">
      <div>
        ...
        <p className="text-sm text-ink-faint">{t('profile.subtitle')}</p>
      </div>
      ...
      {/* Editable fields */}
      <div className="bg-surface border-hairline rounded-xl p-5 space-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="block text-sm font-medium text-ink-soft">
            ...
            <select
              ...
              className="mt-1 w-full border rounded-lg px-3 py-2 text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-primary/30"
            >
      ...
      {!isSupplier && (
        <div className="bg-surface border-hairline rounded-xl p-5">
