      ...
      <div className="space-y-6 max-w-4xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-ink-faint mt-1">{t('reportBuilder.subtitle', 'Définissez des rapports personnalisés, exécutez-les à la demande ou planifiez leur envoi par e-mail.')}</p>
            </div>
          </div>
          <button onClick={downloadExecSummary}
            className="inline-flex items-center gap-2 border px-3 py-2 rounded-lg text-sm font-medium text-ink-soft hover:bg-ground">
            <FileText className="w-4 h-4" /> {t('reportBuilder.execSummary', 'Résumé exécutif (PDF)')}
          </button>
        </div>

        <form onSubmit={e => { e.preventDefault(); if (name.trim()) create.mutate() }} className="bg-surface rounded-xl border border-hairline p-5 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead><tr className="text-left text-ink-faint hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reportBuilder.name', 'Nom')}</th>
                <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reportBuilder.datasetCol', 'Données')}</th>
                <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reportBuilder.format', 'Format')}</th>
                <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('reportBuilder.frequencyCol', 'Fréquence')}</th>
                  ...
                  <tr key={d.id} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex items-center gap-3 justify-end">
                        <button onClick={() => openPreview(d)} disabled={previewing === d.id} className="text-ink-faint hover:text-primary" title={t('reportBuilder.preview', 'Aperçu')}>
                        ...
                        <button onClick={() => setDeleteTargetId(d.id)} className="text-ink-faint hover:text-red-500" title={t('app.delete', 'Supprimer')}>
        ...
        {preview && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
            ...
            <div className="bg-surface rounded-xl shadow-xl max-w-3xl w-full max-h-[80vh] flex flex-col"
                 onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-5 py-4 border-b">
                <div>
                  <h2 className="font-semibold text-ink">{preview.def.name}</h2>
                  <span className="text-xs text-ink-faint">{preview.data.dataset} · {preview.data.format}</span>
                </div>
              ...
              <div className="overflow-auto p-5 flex-1">
                {preview.data.rows.length === 0 ? (
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead><tr className="text-left text-ink-faint hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                      {preview.data.columns.map((c, i) => <th key={i} className="px-3 py-2 whitespace-nowrap bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{c}</th>)}
                    </tr></thead>
                    <tbody className="divide-y divide-hairline">
                      {preview.data.rows.map((row, ri) => (
                        <tr key={ri} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
              ...
              <div className="flex items-center justify-between px-5 py-4 border-t">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPreview(null)} className="px-3 py-2 text-sm border rounded-lg hover:bg-ground">{t('app.close', 'Fermer')}</button>
