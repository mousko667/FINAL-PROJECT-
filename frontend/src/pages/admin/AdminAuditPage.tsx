...
function AnomalyPanel() {
  ...
  return (
    <Panel className="border-crit/30 p-4">
      ...
      <ul className="divide-y divide-hairline">
        {anomalies.map((a, i) => (
          <li key={i} className="flex items-center justify-between gap-3 py-2 text-sm">
            ...
            <span className="text-xs text-ink-soft text-right">{a.detail}</span>
          </li>
        ))}
      </ul>
    </Panel>
                ...
                {logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <tr key={log.id} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                      <td className="px-4 py-3 text-xs text-ink-soft whitespace-nowrap num">
                        {(() => {
                      ...
                      <td className="px-4 py-3 text-xs text-ink-soft max-w-xs truncate">
