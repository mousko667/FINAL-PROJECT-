      ...
      {isLoading ? (
        ...
        <Panel className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-ink-faint text-xs font-medium uppercase tracking-wide bg-ground hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('accessRequests.requester')}</th>
                ...
                <th className="px-4 py-2.5 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('accessRequests.action')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-hairline">
              {requests.map((r) => {
                ...
                return (
                  <tr key={r.id} className="align-top hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                      ...
                      {pending ? (
                        <div className="flex flex-col items-end gap-2">
                          <input
                            ...
                            className="w-48 border border-hairline rounded-[4px] px-2 py-1 text-xs focus:ring-2 focus:ring-primary/40 focus:outline-none bg-surface"
                          />
                          <div className="flex gap-2">
                            <button
                            ...
                            <button
                              ...
                              disabled={busy}
                              className="inline-flex items-center gap-1 px-3 py-1.5 bg-red-600 text-white rounded-[4px] text-xs font-medium hover:bg-red-700 disabled:opacity-50"
                            >
