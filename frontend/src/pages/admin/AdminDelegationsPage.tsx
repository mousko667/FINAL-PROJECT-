...
export default function AdminDelegationsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: { response?: { data?: { message?: string } } }) => {
      setFormError(translateApiMessage(err, t) ?? t('admin.delegations.createError', 'Could not create the delegation.'))
    },
  })
                      ...
                      <tr key={d.id} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                        ...
                        <td className="px-4 py-3 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-xs text-crit hover:opacity-80 disabled:opacity-50"
                          >
