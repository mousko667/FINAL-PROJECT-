      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
            ...
            <p className="text-xs text-ink-faint">{t('retentionDisposition.emptyHint', 'Tous les documents archivés sont dans leur durée de rétention.')}</p>
          </div>
        ) : (
          <div className="bg-surface rounded-lg border border-hairline overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-ground text-left text-xs text-ink-faint">
                <tr>
                  <th className="px-4 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('retentionDisposition.colFile', 'Fichier')}</th>
                  <th className="px-4 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('retentionDisposition.colInvoice', 'Facture')}</th>
                  <th className="px-4 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('retentionDisposition.colUploaded', 'Déposé le')}</th>
                  <th className="px-4 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('retentionDisposition.colActions', 'Actions')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-hairline">
                {docs.map(doc => (
                  <tr key={doc.id} className="border-t hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                    ...
                    <td className="px-4 py-2">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="px-3 py-1 text-xs rounded-md border hover:bg-ground disabled:opacity-60">
            ...
            <div className="bg-surface rounded-lg shadow-lg max-w-md w-full p-6 space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t">
                <button
                  ...
                  className="px-4 py-2 text-sm rounded-[4px] border hover:bg-ground">
                  {t('retentionDisposition.cancel', 'Annuler')}
                </button>
                <button
                  onClick={() => disposition.mutate({ id: purgeTarget.id, value: 'PURGED' })}
                  disabled={disposition.isPending}
                  className="px-4 py-2 text-sm rounded-[4px] bg-red-600 text-white hover:bg-red-700 disabled:opacity-60">
