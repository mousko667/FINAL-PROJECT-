...
export default function AdminAnnouncementsPage() {
  ...
  const inputCls = 'w-full border border-hairline rounded-[4px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 bg-surface'
      ...
      <Panel>
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left text-ink-faint text-xs font-medium uppercase tracking-wide bg-ground hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
              <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.announcements.titleField', 'Titre')}</th>
              <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.announcements.severity', 'Niveau')}</th>
              <th className="px-4 py-2.5 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.announcements.statusCol', 'Actif')}</th>
            ...
            <tbody className="divide-y divide-hairline">
              {announcements.map(a => (
                <tr key={a.id} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                  ...
                  <td className="px-4 py-2.5 text-ink-soft">{a.severity}</td>
                  <td className="px-4 py-2.5">
                    <button onClick={() => toggle.mutate({ id: a.id, active: !a.active })}
                      className={`inline-flex items-center rounded-[4px] px-2 py-0.5 text-xs font-medium ${a.active ? 'bg-pos-bg text-pos' : 'bg-ground text-ink-soft border border-hairline'}`}>
