      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="bg-surface rounded-lg border border-hairline p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-ground border rounded-[4px] p-4">
              <div>
                <span className="block text-xs text-ink-faint">{t('retentionPolicy.lastSweep', 'Dernier balayage')}</span>
                <span className="font-medium text-ink">{formatDate(policy?.lastSweepAt)}</span>
              </div>
              <div>
                <span className="block text-xs text-ink-faint">{t('retentionPolicy.lastFlagged', 'Documents signalés')}</span>
            ...
            {saveMutation.isSuccess && (
              <p className="text-sm text-pos bg-pos/10 p-3 rounded-md border border-green-200">
