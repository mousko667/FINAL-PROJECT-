...
export function IntegrationConnectors() {
  ...
  const statusCls = (s: string | null) =>
    s === 'UP' ? 'bg-pos/10 text-pos' : s === 'DOWN' ? 'bg-crit/10 text-red-800' : 'bg-ground text-ink-faint'
  ...
  return (
    <div className="bg-surface rounded-xl border border-hairline p-5">
      ...
      <p className="text-sm text-ink-faint mb-4">{t('admin.connectors.subtitle', 'Connectez des systèmes externes (ERP, comptabilité, banque, GED). Le type MOCK simule une connexion.')}</p>
      ...
      {isLoading ? (
        ...
        <table className="w-full text-sm">
          <thead><tr className="text-left text-ink-faint hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
            <th className="px-3 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.connectors.name', 'Nom')}</th>
            <th className="px-3 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">Type</th>
            <th className="px-3 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.connectors.status', 'Statut')}</th>
            <th className="px-3 py-2 bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">{t('admin.connectors.sync', 'Synchronisation')}</th>
            <th className="px-3 py-2 text-right bg-ground text-xs font-medium uppercase tracking-wide text-ink-faint">Actions</th>
          </tr></thead>
          <tbody className="divide-y divide-hairline">
            {connectors.map(c => (
              <tr key={c.id} className="hover:bg-[color-mix(in_srgb,hsl(var(--gold-deep))_5%,transparent)]">
                ...
                <td className="px-3 py-2">
                  ...
                  {c.lastSyncStatus && (
                    <div className={`text-xs mt-0.5 ${c.lastSyncStatus === 'SUCCESS' ? 'text-pos' : 'text-red-500'}`}>
                ...
                <td className="px-3 py-2 text-right">
                  <div className="flex items-center gap-3 justify-end">
                    ...
                    <button onClick={() => setDeleteTargetId(c.id)} className="text-ink-faint hover:text-red-500" title={t('app.delete', 'Supprimer')}>
