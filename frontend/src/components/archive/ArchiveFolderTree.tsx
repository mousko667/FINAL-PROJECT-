  ...
  const renderFolderForm = (isEdit: boolean, parentId?: string) => (
    <div className="pl-6 pr-2 py-2 border-l-2 border-primary ml-2 my-1 bg-ground rounded-r-lg">
      <input
        autoFocus
        type="text"
        className="w-full text-sm border border-hairline rounded px-2 py-1 mb-2 outline-none focus:ring-1 focus:ring-primary"
      ...
      <div className="flex gap-2 justify-end">
        <button
          ...
          className="text-xs text-ink-soft hover:text-ink-soft"
        >
            ...
            <li key={folder.id}>
              {isEditing ? (
                ...
                <div
                  ...
                  className={`group flex items-center justify-between py-1.5 px-2 rounded-[4px] cursor-pointer transition-colors ${isSelected ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-ground text-ink-soft'}`}
                ...
                >
                  <div className="flex items-center gap-1.5 flex-1 min-w-0">
                    ...
                    <span className="text-[10px] bg-surface border border-hairline px-1.5 py-0.5 rounded-full text-ink-soft ml-1">
                      {folder.invoiceCount}
                    </span>
                  </div>
                  ...
                  {isAdmin && (
                    <div className="hidden group-hover:flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="p-1 text-ink-faint hover:text-crit hover:bg-crit-bg rounded"
                      >
      ...
      <div className="p-4 border-b flex items-center justify-between bg-surface">
        ...
        {isAdmin && !isCreating && (
          <button
            ...
            className="p-1.5 text-primary hover:bg-primary/10 rounded-[4px] transition-colors"
            title={t('archiveFolders.new')}
          >
      ...
      <div className="p-2 flex-1 overflow-y-auto">
        ...
        <div className="space-y-0.5 mb-4">
          <button
            ...
            className={`w-full flex items-center gap-2 py-1.5 px-2 rounded-[4px] text-sm transition-colors ${selectedFolderId === null ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-ground text-ink-soft'}`}
          >
            <ArchiveFolderIcon />
          ...
          <button
            ...
            className={`w-full flex items-center gap-2 py-1.5 px-2 rounded-[4px] text-sm transition-colors ${selectedFolderId === 'NONE' ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-ground text-ink-soft'}`}
          >
