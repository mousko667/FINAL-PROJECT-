...
const MIME_ICONS: Record<string, React.ReactNode> = {
  'application/pdf': <FileText className="w-5 h-5 text-crit" />,
  'image/png': <Image className="w-5 h-5 text-info" />,
  'image/jpeg': <Image className="w-5 h-5 text-info" />,
  ...
  return (
    <div className="space-y-3">
      ...
      <label
        ...
        onDrop={handleDrop}
        className={cn(
          'flex flex-col items-center justify-center w-full h-44 border-2 border-dashed rounded-[4px] cursor-pointer transition-colors',
          dragOver ? 'border-primary bg-primary/5' : 'border-hairline-strong hover:bg-ground',
          disabled && 'opacity-50 cursor-not-allowed'
        )}
      >
        <Upload className={cn('w-10 h-10 mb-2', dragOver ? 'text-primary' : 'text-ink-faint')} />
      ...
      {errors.length > 0 && (
        <ul className="space-y-1">
          {errors.map((e, i) => (
            <li key={i} className="text-xs text-crit bg-crit-bg px-3 py-1.5 rounded-[4px]">
              {e}
            </li>
          ))}
        </ul>
        ...
        <ul className="space-y-2">
          {files.map((f, i) => (
            <li
              key={i}
              id={`doc-item-${i}`}
              className="flex items-center gap-3 px-3 py-2.5 bg-ground border border-hairline rounded-[4px]"
            >
              {getIcon(f)}
              ...
              <div className="w-16 h-1.5 bg-pos-bg rounded-full overflow-hidden">
                <div className="h-full w-full bg-pos rounded-full" />
              </div>
              {!disabled && (
                <button
                  id={`btn-remove-doc-${i}`}
                  ...
                  className="p-1 text-ink-faint hover:text-crit transition-colors"
                >
