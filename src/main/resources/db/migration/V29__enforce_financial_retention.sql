-- Enforce the 10-year minimum retention rule for financial records.
-- These records may be soft-deleted where the application allows it, but hard deletion
-- is blocked until the configured legal retention horizon has elapsed.
...
DROP TRIGGER IF EXISTS trg_invoices_retention_delete ON invoices;
CREATE TRIGGER trg_invoices_retention_delete
    BEFORE DELETE ON invoices
    FOR EACH ROW EXECUTE FUNCTION prevent_financial_delete_before_10_years();

DROP TRIGGER IF EXISTS trg_invoice_documents_retention_delete ON invoice_documents;
CREATE TRIGGER trg_invoice_documents_retention_delete
    BEFORE DELETE ON invoice_documents
    FOR EACH ROW EXECUTE FUNCTION prevent_financial_delete_before_10_years();

DROP TRIGGER IF EXISTS trg_invoice_status_history_retention_delete ON invoice_status_history;
CREATE TRIGGER trg_invoice_status_history_retention_delete
    BEFORE DELETE ON invoice_status_history
    FOR EACH ROW EXECUTE FUNCTION prevent_financial_delete_before_10_years();

DROP TRIGGER IF EXISTS trg_payments_retention_delete ON payments;
CREATE TRIGGER trg_payments_retention_delete
    BEFORE DELETE ON payments
    FOR EACH ROW EXECUTE FUNCTION prevent_financial_delete_before_10_years();

DROP TRIGGER IF EXISTS trg_remittance_advice_retention_delete ON remittance_advice;
