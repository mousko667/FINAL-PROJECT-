-- B1 (M4): validation checklist templates. An admin defines reusable templates (named lists of
-- check items). A template is either global (department_id NULL) or scoped to one department.
-- During validation the applicable template is shown; the validator's answers are persisted per
-- invoice (interactive, traced — but they do NOT block the workflow).

CREATE TABLE checklist_templates (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name          VARCHAR(255) NOT NULL,
    department_id UUID REFERENCES departments (id) ON DELETE CASCADE,  -- NULL = global template
