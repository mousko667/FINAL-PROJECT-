...
| 8 | Rejection reason selection | ✅ | **Liste de motifs prédéfinis obligatoire** (dropdown) + détail libre optionnel (obligatoire ≥10 car. si motif = `AUTRE`). Enum `RejectionReasonCode` (6 motifs), endpoint `GET /workflow/rejection-reasons` (libellés i18n FR/EN), motif+détail composés `[CODE] détail` dans `rejectionReason`. **Fait (C1, 2026-06-19)** : `rejectionReasons_returnsTranslatedOptions_fr`, `reject_withCodeAndDetail_persistsBracketedReason`, validation `AUTRE`/code null → 400 ; test front dropdown. **Fix (2026-07-18, N2/PROB-117)** : le `RejectionReasonGuard` appliquait le seuil de 10 caractères sur la chaîne **composée** `[CODE]`, si bien que rejeter pour `DOUBLON` (`[DOUBLON]`=9 car.) échouait en HTTP 400. Le guard ne valide plus que la **présence** d'un motif (un code prédéfini est un motif structuré) ; la seule contrainte de longueur — détail libre ≥10 car. pour le code `AUTRE` — reste dans `ApprovalController.reject`. Test : `reject_withPredefinedCodeAndNoDetail_succeeds`. |
| 9 | Re-submission workflow for rejected invoices | ✅ | Endpoint `/invoices/{id}/resubmit` (REJETE → SOUMIS). **Durcissement (2026-07-18, N1/PROB-118)** : la resoumission exige désormais une correction réelle depuis le rejet — `invoices.version_at_rejection` (V48) est capturé au passage à REJETE et `ResubmissionVersionGuard` refuse le RESUBMIT tant que `version` n'a pas dépassé cette valeur ; la garde doublon+matching de `sendEvent` couvre désormais `SUBMIT || RESUBMIT`, donc un MISMATCH resoumis exige toujours l'override DAF/ADMIN. Tests : `ApprovalControllerTest.resubmit_withoutCorrection_isRejected` / `resubmit_afterCorrection_succeeds`, `InvoiceStateMachineServiceTest.resubmit_WithPurchaseOrderAndUnevaluableMatching_ThrowsWorkflowException`. |
