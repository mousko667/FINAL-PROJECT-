...
### [PROB-095] 29 clés i18n frontend manquantes (`supplier.onboarding.*`, `admin.backups.*`, `archiveFolders.*`) + 5 appels `common.*` vers un namespace inexistant

- **Catégorie :** Frontend / Internationalisation (i18n)
- **Sévérité :** 🟠 Important (plusieurs écrans affichaient soit la clé brute non résolue, soit du texte anglais figé passé en fallback, en violation de l'exigence « français langue principale de l'UI » ; les appels `common.*` ne résolvaient jamais que le fallback ou rien du tout, `common` n'ayant jamais existé comme namespace racine).
- **Découvert :** 2026-07-04 — Audit correctif (Task 8, MAJEUR-5) : grep de `t('supplier.onboarding`, `t('admin.backups`, `t(\`archiveFolders`, `duplicateWarning` dans `frontend/src` puis résolution de chaque clé trouvée contre `fr.json`/`en.json` via un script Node (getter par chemin pointé). Trois namespaces entiers étaient absents (`supplier.onboarding.*` dans `SupplierOnboardingPage.tsx`, `admin.backups.*` dans `AdminBackupsPage.tsx` et `Sidebar.tsx`, `archiveFolders.*` dans `ArchiveFolderTree.tsx`/`AssignFolderModal.tsx`/`ArchivePage.tsx`) et `invoice.duplicateWarning` n'existait qu'en variantes pluralisées (`duplicateWarning_one`/`duplicateWarning_other`), sans clé de base résolvable par un lookup direct. Par ailleurs, 5 sites d'appel (pas 4 comme initialement supposé — `AdminBackupsPage.tsx` en contenait deux, lignes 115 et 231) utilisaient `common.actions`/`common.cancel`/`common.loading`, un namespace racine `common` qui n'a jamais existé dans `fr.json`/`en.json` (le namespace correct, déjà peuplé, est `app.*`).
- **Cause racine :** (1) trois écrans (onboarding fournisseur, sauvegardes admin, dossiers d'archive) ont été livrés avec leurs clés `t(...)` câblées dans le JSX mais sans jamais faire l'aller-retour de synchronisation vers `fr.json`/`en.json` — un top-level `backups.*` existait déjà dans le JSON mais sous la mauvaise racine (`backups.*` au lieu de `admin.backups.*`), donc invisible à `AdminBackupsPage`. (2) le préfixe `common.` a été utilisé par erreur (probablement copié d'un autre projet/gabarit) au lieu du préfixe `app.` réellement utilisé dans ce codebase pour les libellés génériques (Actions/Annuler/Chargement).
- **Solution appliquée :** ajout de `admin.backups.*` (10 clés, dupliquant intentionnellement le contenu du `backups.*` racine existant qui reste utilisé ailleurs et n'a pas été touché), `supplier.onboarding.*` (11 clés dont `step1`/`step2`/`step3` pour la boucle `t(\`supplier.onboarding.step${item}\`)`), et du nouveau namespace racine `archiveFolders.*` (7 clés), en FR et EN, strictement symétriques (vérifié par script de parcours récursif des deux arbres). Ajout d'une clé de base `invoice.duplicateWarning` (identique au texte de `duplicateWarning_one`) à côté des variantes `_one`/`_other` existantes, pour que le lookup direct par chemin pointé résolve sans dépendre de la pluralisation i18next — sans rien changer au comportement runtime existant (i18next continue de préférer `_one`/`_other` dès qu'un `count` est passé). Les 5 sites d'appel `common.actions`/`common.cancel`/`common.loading` ont été corrigés vers `app.actions`/`app.cancel`/`app.loading` dans `MatchingLineResolveModal.tsx`, `MatchingDetailPage.tsx`, `DepartmentAccessPage.tsx` et `AdminBackupsPage.tsx` (×2).
- **Règle préventive :** quand un écran est livré avec des clés `t('namespace.key')` câblées dans le JSX, la PR DOIT inclure le diff correspondant dans `fr.json` ET `en.json` dans le même commit — un grep de `t\(['"]<namespace>` suivi d'une résolution programmatique (pas un survol visuel) contre les deux fichiers JSON doit faire partie de la checklist de revue pour tout nouvel écran. Ne jamais introduire un préfixe i18n ad-hoc (`common.*`) sans vérifier d'abord quel est le namespace générique réellement utilisé dans le reste du codebase (ici `app.*`).
- **Fichiers modifiés :** `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/components/matching/MatchingLineResolveModal.tsx`, `frontend/src/pages/matching/MatchingDetailPage.tsx`, `frontend/src/pages/admin/DepartmentAccessPage.tsx`, `frontend/src/pages/admin/AdminBackupsPage.tsx`.

---

### [PROB-096] `ExportMenu.download()` n'avait aucun `catch` : rejet non géré + risque de télécharger un blob d'erreur comme si c'était le fichier

- **Catégorie :** Frontend / Gestion d'erreurs
- **Sévérité :** 🟠 Important (composant partagé utilisé par tous les exports CSV/Excel/PDF de l'application — matching, paiements, audit, fournisseurs, archives — donc le défaut était présent partout où `ExportMenu` est monté).
- **Découvert :** 2026-07-04 — Audit correctif (Task 9, MAJEUR-7). `ExportMenu.tsx` `download()` enveloppait l'appel `apiClient.get(..., { responseType: 'blob' })` dans un `try { ... } finally { setBusy(null); setOpen(false) }` sans aucun `catch`. Sur un 403/500/erreur réseau, la promesse rejetée n'était interceptée nulle part : rejet de promesse non géré, aucun retour visuel pour l'utilisateur (le menu se refermait silencieusement comme en cas de succès), et si le serveur répondait avec un corps d'erreur au lieu de rejeter (ex. 200 avec JSON d'erreur typé `blob`), ce corps aurait été télécharge tel quel comme si c'était le fichier demandé.
- **Cause racine :** le `try/finally` a été écrit pour garantir la remise à zéro de l'état `busy`/`open` sans jamais couvrir le cas d'échec du téléchargement — un oubli du chemin malheureux (« happy path only ») sur un composant réseau qui peut légitimement échouer (permissions, timeout, format non supporté côté serveur).
- **Solution appliquée :** ajout d'un état `const [error, setError] = useState<string | null>(null)`, remis à `null` en tête de `download()` puis peuplé dans un nouveau bloc `catch { setError(t('app.exportError', ...)) }` place avant le `finally` existant (qui continue de fermer le menu et de désarmer `busy` dans tous les cas). Le message d'erreur est rendu **en dehors** du bloc `{open && ...}` (juste sous le bouton), avec `role="alert"`, pour rester visible même une fois le menu refermé par le `finally`. Aucune balise `<a>` n'est créée/cliquée et `URL.createObjectURL` n'est jamais appelé si `apiClient.get` rejette, car ces lignes sont désormais strictement après l'`await` dans le `try` et un rejet saute directement au `catch`. Nouvelle clé i18n `app.exportError` ajoutée à `fr.json`/`en.json` (namespace existant `app.*`). Tests TDD dans `ExportMenu.test.tsx` : cas échec (mock `apiClient.get` rejeté → aucun `createElement('a')`, aucun `createObjectURL`, message `role="alert"` visible) et cas succès en garde de non-régression (mock résolu → `createObjectURL` + `click()` appelés, aucune alerte). RED confirmé avant le fix (le test échec échouait par timeout sur l'alerte absente + rejet non géré loggé par Vitest), GREEN après.
- **Règle préventive :** tout `try/finally` autour d'un appel réseau (`apiClient.*`) DOIT avoir un `catch` qui distingue explicitement l'échec du succès et donne un retour à l'utilisateur — un `finally` seul masque l'échec en exécutant le même nettoyage (fermeture de menu, reset d'état `busy`) que le succès, ce qui rend l'échec indiscernable dans l'UI. Pour un flux de téléchargement de fichier binaire (`responseType: 'blob'`), ne jamais créer/cliquer l'ancre `<a>` avant d'être sûr que la réponse est bien le fichier attendu et pas un corps d'erreur.
- **Fichiers modifiés :** `frontend/src/components/ui/ExportMenu.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/test/components/ExportMenu.test.tsx`.

### [PROB-097] Le compte `admin` semé (V5) avait `mfa_verified=false` / `mfa_secret=NULL` → `MfaSetupEnforcementFilter` bloquait toute action admin (`mfa_setup_required`)
- **Catégorie :** Sécurité / Authentification (MFA) / Migration Flyway
- **Sévérité :** 🟠 Majeur (MAJEUR-6 de l'audit : le compte administrateur bootstrap était inutilisable dès un environnement neuf ou un re-seed — aucune action ne passait, le filtre renvoyant systématiquement `mfa_setup_required`).
- **Découvert :** 2026-07-04 — Audit correctif (Task 10). `V5__seed_roles_and_admin.sql` insère `admin` sans renseigner les colonnes MFA (`INSERT INTO users (username, password_hash, email, first_name, last_name, preferred_lang, is_active)`), donc elles retombent sur leurs défauts V3 : `mfa_enabled=false`, `mfa_verified=false`, `mfa_secret=NULL`. Or `MfaSetupEnforcementFilter.doFilter` bloque tout rôle non-SUPPLIER (`requiresMandatoryMfa`) qui n'est pas « truly verified » : en dev (`app.security.mfa.enforce-secret-check=false`) `trulyVerified = user.isMfaVerified()`, qui est `false` → réponse 400 `mfa_setup_required` sur toute requête hors `/auth/mfa/setup|confirm`. Le compte admin était donc brické à froid. Des correctifs manuels en base avaient été appliqués lors de sessions antérieures (cf. mémoire mfa-enrollment-and-seed) mais ne survivent pas à un re-seed / une base fraîche.
- **Cause racine :** un état d'environnement (le compte admin doit être exploitable dès le seed) a été rattrapé à la main en base au lieu d'être fixé dans une migration versionnée. Le seed V5 lui-même est immuable (checksum verrouillé, règle PROB-009), donc la remise en état doit passer par une nouvelle migration idempotente, pas par du SQL manuel ni par la modification de V5.
- **Solution appliquée :** nouvelle migration `V43__fix_admin_account.sql`, `UPDATE users SET mfa_verified = true, mfa_secret = NULL, mfa_enabled = false WHERE username = 'admin'`. Idempotente (clé sur `username`, no-op si déjà conforme), cohérente avec le mode dev `enforce-secret-check=false` où `mfa_verified=true` suffit à débloquer le filtre ; `mfa_secret` remis à NULL pour repartir d'un état ré-enrôlable propre (pas de secret figé/dérivé), `mfa_enabled=false` cohérent avec « compte vierge, MFA à configurer ». Aucun secret TOTP en clair écrit. Le `password_hash` déjà posé en V5 est correct et n'est pas touché. Suite backend inchangée : 553/0/0 (la migration ne s'exécute pas dans le profil de test — H2 + `flyway.enabled=false` + `ddl-auto=create-drop` —, elle s'applique au démarrage réel de l'app sur PostgreSQL ; validité PostgreSQL vérifiée statiquement : colonnes `mfa_verified`/`mfa_secret`/`mfa_enabled` existent depuis V3, `username` unique).
- **Règle préventive :** un correctif d'état d'environnement (données de seed, compte bootstrap) appartient TOUJOURS à une migration Flyway idempotente, jamais à du SQL manuel en base — sinon il disparaît au prochain re-seed. Ne jamais modifier une migration déjà appliquée (V5) : créer la migration suivante. Quand une ligne semée doit satisfaire un filtre de sécurité (ici MFA), vérifier le seed contre la logique réelle du filtre (`enforce-secret-check` dev vs prod) plutôt que de supposer les valeurs par défaut suffisantes.
- **Fichiers modifiés :** `src/main/resources/db/migration/V43__fix_admin_account.sql` (nouveau).

---

### [PROB-098] `AccessRequestService.create` ne rejetait que `ROLE_SUPPLIER` en self-service : `ROLE_ADMIN`/`ROLE_DAF` demandables par n'importe quel staff (escalade de privilège)

- **Catégorie :** Sécurité / Autorisation (séparation des devoirs, IAM)
- **Sévérité :** 🟠 Majeur (MAJEUR-10 de l'audit : un utilisateur staff, par exemple un ASSISTANT_COMPTABLE, pouvait soumettre une demande d'accès pour `ROLE_ADMIN` ou `ROLE_DAF` ; une approbation par inadvertance d'un reviewer aurait accordé un rôle d'administration ou un rôle financier approbateur en violation directe de la règle « ADMIN ne doit jamais avoir accès aux données financières » et « DAF est un rôle d'approbation financière »).
- **Découvert :** 2026-07-04 — Audit correctif (Task 11, MAJEUR-10). `AccessRequestService.create(UUID, AccessRequestCreateRequest)` (l.47-73) ne testait que `if (ROLE_SUPPLIER.equals(requestedRole))` avant de vérifier l'existence du rôle en base puis de créer la demande `PENDING`. Aucun contrôle n'empêchait `requestedRole = "ROLE_ADMIN"` ou `"ROLE_DAF"` de passer ce filtre : la demande était créée normalement et n'attendait plus qu'une approbation (`review`) pour que `grantRole` attache le rôle privilégié au demandeur.
- **Cause racine :** le contrôle de non-auto-demande n'avait été pensé qu'autour du cas `ROLE_SUPPLIER` (rôle géré exclusivement par le portail fournisseur) au moment de la conception du workflow (P11-17), sans anticiper que d'autres rôles du référentiel (`ROLE_ADMIN`, `ROLE_DAF`) sont également sensibles et ne doivent jamais transiter par un flux d'auto-service, même avec approbation humaine — un seul rôle exclu au lieu d'une liste de rôles non auto-demandables.
- **Solution appliquée :** remplacement de la constante unique `ROLE_SUPPLIER` par un ensemble `private static final Set<String> NON_SELF_REQUESTABLE_ROLES = Set.of("ROLE_SUPPLIER", "ROLE_ADMIN", "ROLE_DAF")`, vérifié par `NON_SELF_REQUESTABLE_ROLES.contains(requestedRole)` **avant** la recherche du rôle en base (`roleRepository.findByName`), afin qu'un rôle privilégié soit rejeté même s'il existe bien dans le référentiel — l'ordre des contrôles dans `create` est préservé. Le message d'exception reste un littéral anglais brut inchangé dans sa forme (`"Role cannot be requested via self-service: " + requestedRole`), **volontairement pas une clé i18n** : les 5 `ValidationException` sœurs de cette méthode (`Unknown role: ...`, `You already have this role: ...`, `A pending request for this role already exists`, etc.) sont déjà des littéraux anglais, et `GlobalExceptionHandler.handleCustomValidationException` renvoie `ex.getMessage()` tel quel au client, sans jamais passer par un `MessageSource` — ajouter une clé `messages_fr.properties`/`messages_en.properties` ici aurait produit une clé morte, jamais résolue sur ce chemin. Tests TDD ajoutés dans `AccessRequestServiceTest` (nouveau fichier, mocks Mockito sur `AccessRequestRepository`/`UserRepository`/`RoleRepository`/`AccessRequestMapper`, calqué sur le style de `DelegationServiceTest`) : `create` avec `requestedRole="ROLE_ADMIN"` et `"ROLE_DAF"` lève `ValidationException` avec message contenant `"self-service"` (assertion ciblée pour ne pas confondre avec le rejet « Unknown role » — `verifyNoInteractions(roleRepository)` prouve que le rejet survient bien avant la recherche du rôle), et un rôle bénin (`ROLE_VALIDATEUR_N1_DRH`) franchit le contrôle de liste noire et atteint la sauvegarde. RED confirmé avant le fix (ADMIN/DAF acceptés), GREEN après. Effet de bord détecté et corrigé : `AccessRequestServiceIntegrationTest` (préexistant) utilisait `ROLE_DAF` comme rôle générique « bénin » dans 5 méthodes de test sans rapport avec la liste noire (persistance, doublon, approbation, rejet, double revue) — remplacé par `ROLE_ASSISTANT_COMPTABLE` (rôle non privilégié réel du référentiel) pour rester conforme à la nouvelle règle de sécurité sans perdre la couverture d'intégration visée par ces tests.
- **Règle préventive :** un contrôle de sécurité de type « liste noire/liste blanche de rôles auto-demandables » doit toujours être exprimé comme un ensemble (`Set`) explicite et vérifié **avant** toute recherche d'existence en base, jamais comme un test d'égalité isolé sur un seul cas connu au moment de l'écriture — un nouveau rôle sensible ajouté plus tard au référentiel ne doit pas pouvoir contourner le contrôle par simple absence de mise à jour de la liste. Les rôles `ROLE_ADMIN` et `ROLE_DAF` ne doivent jamais être auto-demandables via un workflow libre-service, quelle que soit l'évolution de ce module (séparation des devoirs). Par ailleurs, quand une méthode de service lève déjà des `ValidationException` en littéraux anglais bruts non résolus par `MessageSource` (vérifier `GlobalExceptionHandler` avant d'ajouter une clé i18n), tout nouveau message dans la même méthode doit suivre le même style plutôt que d'introduire une clé i18n qui ne sera jamais résolue sur ce chemin.
- **Fichiers modifiés :** `src/main/java/com/oct/invoicesystem/domain/access/service/AccessRequestService.java`, `src/test/java/com/oct/invoicesystem/domain/access/service/AccessRequestServiceTest.java` (nouveau), `src/test/java/com/oct/invoicesystem/domain/access/service/AccessRequestServiceIntegrationTest.java`.

---

### [PROB-099] `IntegrationConnector.config` stockait en clair la configuration des connecteurs ERP/BANKING/DMS, potentiellement des identifiants ou clés d'API

- **Catégorie :** Sécurité / Chiffrement des données au repos
- **Sévérité :** 🟠 Majeur (MAJEUR-13 de l'audit : `config` peut contenir des paramètres de connexion à des systèmes externes — identifiants, jetons, clés d'API — pour les connecteurs ERP, ACCOUNTING, BANKING, DMS ; un accès en lecture directe à la base de données (dump, fuite, accès DBA non autorisé) exposait ces secrets en clair, contrairement à `Supplier.bankDetails` qui bénéficie déjà du chiffrement au repos).
- **Découvert :** 2026-07-04 — Audit correctif (Task 12, MAJEUR-13). `IntegrationConnector.java` (l.37-38) déclarait `config` avec `@Column(length = 4000)` sans aucun `@Convert`, alors que `Supplier.bankDetails`, donnée de sensibilité comparable, utilise `@Convert(converter = EncryptionAttributeConverter.class)` depuis l'origine du projet. Aucune ligne de connecteur n'est semée par une migration Flyway ni par un seeder Java : il n'existait donc aucune donnée en clair préexistante à rechiffrer.
- **Cause racine :** l'entité `IntegrationConnector` (M12) a été conçue après `Supplier` sans reprendre systématiquement le pattern de chiffrement déjà établi pour les champs sensibles ; le champ `config`, de nature libre (JSON/texte selon le type de connecteur), n'a pas été identifié comme candidat au chiffrement au moment de sa création, alors qu'il peut structurellement contenir des secrets au même titre que `bankDetails`.
- **Solution appliquée :** ajout de `@Convert(converter = EncryptionAttributeConverter.class)` sur `config`, en miroir exact du pattern `Supplier.bankDetails`, avec `@Column(name = "config", columnDefinition = "TEXT")` — colonne élargie en `TEXT` (au lieu de conserver `VARCHAR(4000)`) car le chiffrement AES-GCM suivi d'un encodage Base64 produit un texte chiffré environ 1,5 à 2 fois plus long que le texte en clair, et une colonne à longueur fixe risquerait un dépassement pour une configuration proche de la limite. Migration `V44__encrypt_integration_connector_config.sql` ajoutée (`ALTER TABLE integration_connectors ALTER COLUMN config TYPE TEXT`), un `ALTER COLUMN ... TYPE TEXT` étant un élargissement sans perte valide en PostgreSQL depuis une `VARCHAR`. Aucune ligne existante à migrer/rechiffrer (aucun seed, aucune donnée de dev connue). Test ajouté `IntegrationConnectorEncryptionTest` (`@SpringBootTest`, `@ActiveProfiles("test")`, `@Transactional`) : persiste un connecteur avec `config = "secret-token-12345"`, `flush()`/`clear()` du contexte de persistance, recharge via le repository et vérifie l'égalité (chiffrement transparent en lecture), puis exécute une requête native (`EntityManager.createNativeQuery`) sur la colonne brute `config` et vérifie qu'elle est différente du texte en clair et préfixée `GCM:` (format de sortie de `EncryptionUtil.encrypt`). RED confirmé avant le fix (assertion sur la colonne brute échouait, `config` étant stocké en clair), GREEN après. Aucun test préexistant (`IntegrationConnectorServiceTest`, tous unitaires avec `@Mock`/`@InjectMocks`, sans persistance réelle) n'a été affecté par le changement, le convertisseur étant transparent au niveau entité.
- **Règle préventive :** tout champ d'entité susceptible de contenir des identifiants, secrets, jetons ou clés d'API (configuration de connecteur, coordonnées bancaires, clés d'API tierces, etc.) DOIT utiliser `@Convert(converter = EncryptionAttributeConverter.class)` associé à une colonne `TEXT` (jamais une longueur fixe, en raison du surcoût de taille du chiffrement AES-GCM + Base64). Lors de la création d'un nouveau champ de ce type, le comparer systématiquement au pattern de référence `Supplier.bankDetails` plutôt que de réévaluer le besoin de chiffrement au cas par cas.
- **Fichiers modifiés :** `src/main/java/com/oct/invoicesystem/domain/webhook/model/IntegrationConnector.java`, `src/main/resources/db/migration/V44__encrypt_integration_connector_config.sql` (nouveau), `src/test/java/com/oct/invoicesystem/domain/webhook/repository/IntegrationConnectorEncryptionTest.java` (nouveau).

### [PROB-100] Le `budget` par département était exposé sur le `DepartmentDTO` public (`GET /departments`) → fuite de donnée financière aux rôles non financiers
- **Catégorie :** Sécurité / Exposition de données (séparation des devoirs, DTO public)
- **Sévérité :** 🟠 Majeur (MAJEUR-9 de l'audit : le budget annuel d'un département — donnée financière réservée au DAF / ASSISTANT_COMPTABLE — était renvoyé à tout utilisateur authentifié, y compris `ROLE_SUPPLIER` et `ROLE_ADMIN`, dès qu'un budget était défini sur le département).
- **Découvert :** 2026-07-04 — Audit correctif (Task 13, MAJEUR-9). `DepartmentDTO` (record) exposait un composant `java.math.BigDecimal budget`, mappé automatiquement depuis l'entité par `DepartmentMapper.toDto(Department)`. Or `DepartmentController` `GET /departments` et `GET /departments/{id}` ne sont gardés que par `isAuthenticated()` (aucune restriction de rôle), donc n'importe quel compte connecté — fournisseur ou admin inclus — recevait le budget dans la réponse. Latence : nul tant qu'aucun budget n'est défini (colonne nullable), mais fuite immédiate dès qu'un DAF renseigne un budget via `PUT /departments/{id}`.
- **Cause racine :** un champ financier (`budget`, ajouté en P11-52 / REQ-20 pour le rapport budget-vs-réalisé) a été placé sur le DTO de lecture générique du département, partagé par des endpoints ouverts à tous les rôles authentifiés, au lieu d'être confiné au chemin financier restreint. Le DTO de lecture publique et la donnée financière n'auraient jamais dû partager la même surface.
- **Solution appliquée :** suppression du composant `budget` du record `DepartmentDTO` (+ commentaire d'avertissement anti-régression sur le record). Aucun chemin légitime n'est cassé : (1) l'écriture du budget passe par `DepartmentUpdateRequest.budget` (admin) → `DepartmentService.setBudget`, inchangée ; (2) la lecture financière passe par le rapport `budget-vs-actual` (`ReportController`, gated DAF/ASSISTANT_COMPTABLE), dont `ReportServiceImpl.getBudgetVsActual` lit `dept.getBudget()` **directement sur l'entité**, jamais via `DepartmentDTO` ; (3) le mapping `toEntity(DepartmentCreateRequest)` conserve son `@Mapping(target="budget", ignore=true)` (la cible est l'entité, pas le DTO). Deux constructions manuelles de `DepartmentDTO` dans `DepartmentControllerTest` ont eu leur argument `budget` (null) retiré pour compiler. Test de non-régression `DepartmentMapperBudgetLeakTest` : une entité `Department` avec `budget=1 500 000` mappée par le vrai `DepartmentMapper` produit un DTO dont la sérialisation JSON ne contient AUCUN champ `budget` (réintroduire le champ ferait échouer ce test). Suite complète 558/0/0.
- **Règle préventive :** ne jamais placer une donnée financière ou sensible (budget, montants, coûts, marges) sur un DTO de lecture servi par un endpoint garde seulement `isAuthenticated()` / ouvert à des rôles non financiers. Confiner ces données à un DTO/endpoint restreint gated aux rôles financiers (DAF / ASSISTANT_COMPTABLE), ou les lire directement sur l'entité dans le service financier. Vérifier systématiquement le niveau d'autorisation de TOUS les endpoints qui renvoient un DTO avant d'y ajouter un champ sensible — un record partagé propage le champ à tous ses consommateurs d'un coup.
- **Fichiers modifiés :** `src/main/java/com/oct/invoicesystem/domain/department/dto/DepartmentDTO.java`, `src/test/java/com/oct/invoicesystem/domain/department/controller/DepartmentControllerTest.java`, `src/test/java/com/oct/invoicesystem/domain/department/mapper/DepartmentMapperBudgetLeakTest.java` (nouveau).

### [PROB-101] Le nom de fichier de restauration de sauvegarde n'était pas assaini → traversée de chemin (path traversal) possible sur `BackupService.restoreBackup(filename)`
- **Catégorie :** Sécurité / Traversée de chemin (path traversal, entrée non validée sur un chemin de fichier)
- **Sévérité :** 🟠 Mineur/durcissement (B-2 de l'audit : endpoint `POST /admin/compliance/backups/{filename}/restore` gardé `ROLE_ADMIN`. L'UI ne restaure que des fichiers listés par `listBackups()` (pas de saisie libre côté utilisateur), donc l'exploitation réelle nécessite déjà un compte ADMIN forgeant une requête à la main. C'est de la défense en profondeur, mais la vuln est réelle : le `filename` provenait d'un `@PathVariable` concaténé directement dans un chemin.).
- **Découvert :** 2026-07-04 — Audit correctif (Task 14, B-2). `BackupController.restoreBackup` (`@PostMapping("/{filename}/restore")`) passait le `@PathVariable String filename` verbatim à `BackupService.restoreBackup(String filename)`, qui construisait `String fullPath = "backups/" + filename;` sans aucune sanitisation. Un `filename` valant `../../etc/passwd` (ou `..\..\x`, ou `a/b`) traversait donc hors du préfixe `backups/`.
- **Cause racine :** entrée utilisateur (segment d'URL) utilisée pour composer un chemin de stockage sans liste blanche de caractères. Piège aggravant : le corps de `restoreBackup` enveloppe tout dans un `try/catch(Exception)` qui **avale** l'exception et renvoie un statut « FAILED » avec HTTP 200 — donc une validation lancée *à l'intérieur* du try aurait été absorbée (200 FAILED) au lieu de propager un 400 propre.
- **Solution appliquée :** validation par liste blanche placée **avant** le bloc try (pour que l'exception se propage au `GlobalExceptionHandler` → HTTP 400, et ne soit pas avalée par le catch) : `if (filename == null || !filename.matches("[A-Za-z0-9._-]+")) throw new ValidationException("error.backup.invalid_filename");`. La regex rejette `..`, `/`, `\`, deux-points de lecteur et tout caractère de chemin, tout en acceptant les noms légitimes (`backup-20260704.sql.gz`). Clé i18n `error.backup.invalid_filename` ajoutée dans `messages_fr.properties` (ISO-8859-1, ici ASCII pur — aucun accent requis) et `messages_en.properties` (UTF-8) — clé et non littéral car l'écran backup peut la `t()` côté front (cf. PROB-006, `GlobalExceptionHandler` renvoie `ex.getMessage()` verbatim). Tests TDD dans `BackupServiceIntegrationTest` : 4 cas de rejet (`../../etc/passwd`, `a/b`, `..\..\x`, `null`) vérifient `ValidationException("error.backup.invalid_filename")` **et** `verify(minioStorageService, never()).download(...)` (le chemin traversé n'atteint jamais le stockage) ; 1 cas positif vérifie qu'un nom valide passe la validation et atteint `download("backups/" + filename)`.
- **Règle préventive :** ne jamais concaténer une entrée utilisateur (path variable, paramètre, nom de fichier uploadé) dans un chemin de stockage/fichier sans liste blanche stricte de caractères (`[A-Za-z0-9._-]+`) rejetant `..`, séparateurs et lettres de lecteur. Placer cette validation **avant** tout bloc try qui avalerait l'exception, afin qu'elle se propage en 400 via `GlobalExceptionHandler` et ne soit pas transformée en faux « succès ».
- **Fichiers modifiés :** `src/main/java/com/oct/invoicesystem/domain/compliance/service/BackupService.java`, `src/main/resources/i18n/messages_fr.properties`, `src/main/resources/i18n/messages_en.properties`, `src/test/java/com/oct/invoicesystem/domain/compliance/service/BackupServiceIntegrationTest.java`.

### [PROB-102] Devise XAF/XOF incohérente + formats nombre/date non forcés sur la locale fr-FR (RT-4 + MAJEUR-F4)
- **Catégorie :** Frontend / Internationalisation (i18n) / UX / cohérence d'affichage
- **Sévérité :** 🟡 Mineur (lot mineurs de l'audit ; RT-4 seul était noté 🟠 « systématique/récurrent » dans le rapport d'audit, mais regroupé ici avec MAJEUR-F4 dans le lot de correctifs mineurs).
- **Découvert :** 2026-07-04 — Audit correctif (Task 15, `docs/AUDIT_GENERAL_2026-07-02.md` L.114 + L.253-254 pour MAJEUR-F4, L.400-405 pour RT-4). **MAJEUR-F4 :** `PurchaseOrdersPage.tsx` initialisait `form.currency` à `'XAF'` alors que le `<select>` de devise ne propose que `XOF`/`EUR`/`USD` — `XAF` n'étant jamais sélectionnable, tout re-render qui dépendait de la valeur initiale non modifiée par l'utilisateur retombait silencieusement sur `XOF` au submit (incohérence entre l'état initial et les options réellement proposées) ; `DashboardPage.tsx` affichait également un libellé « (XAF) » alors que la devise fonctionnelle du système est le Franc CFA BCEAO (XOF), pas le Franc CFA BEAC (XAF). **RT-4 :** 66 sites dans 35 fichiers appelaient `Number(x).toLocaleString()` / `new Date(x).toLocaleDateString()` / `new Date(x).toLocaleString()` **sans argument de locale**, ce qui fait retomber `Intl`/`Date.prototype.toLocaleString` sur la locale par défaut du runtime JavaScript (Node/navigateur), typiquement `en-US` dans un environnement non configuré en français — produisant des dates en `M/D/YYYY`, des horodatages AM/PM, et des montants séparés par des virgules US (`38,900,000`) au lieu du format attendu `JJ/MM/AAAA` et de l'espace de groupement des milliers en français.
- **Cause racine :** (1) **XAF/XOF :** le littéral de devise par défaut a été copié depuis un gabarit générique (Franc CFA BEAC, utilisé en Afrique centrale) sans être aligné sur la devise réellement documentée du projet (Franc CFA BCEAO, XOF, utilisé en Afrique de l'Ouest — OCT est basé à Owendo/Gabon mais le système documente XOF comme devise fonctionnelle), et cette incohérence n'a été détectée nulle part parce qu'aucun test n'asserte la valeur de la devise par défaut. (2) **Formats non localisés :** chaque écran appelait directement les API `Intl`/`Date` du navigateur sans jamais passer de locale explicite, un oubli systématique plutôt qu'une erreur isolée — le projet n'avait aucun helper de formatage partagé (`src/lib/utils.ts` ne contenait que `cn()`), donc chaque développeur a réimplémenté son propre appel `toLocaleString()` sans locale, dans 35 fichiers différents.
- **Solution appliquée :** (a) nouveau helper partagé `frontend/src/lib/format.ts` : `formatAmount(n)` (Intl.NumberFormat('fr-FR'), retourne `'—'` pour null/undefined/vide/NaN), `formatDate(d)` (toLocaleDateString('fr-FR') → JJ/MM/AAAA), `formatDateTime(d)` (toLocaleString('fr-FR') → JJ/MM/AAAA HH:mm, 24h). (b) Remplacement de tous les littéraux `XAF` par `XOF` dans le code (`AgingBucketsWidget.tsx`, `VolumeTrendSection.tsx`, `DashboardPage.tsx`, `ProfilePage.tsx`, `PurchaseOrdersPage.tsx` — le fix MAJEUR-F4 proprement dit à la ligne d'initialisation du formulaire —, `ReportsPage.tsx`) et dans les clés i18n (`fr.json`/`en.json` : `reports.trends.amount`, `reports.amountDue`, `admin.users.approvalLimit`, en FR et EN). (c) Routage de 55 des 66 call-sites de l'inventaire d'audit (`toLocaleString`/`toLocaleDateString`/`toLocaleString` pour date+heure, sans locale) vers `formatAmount`/`formatDate`/`formatDateTime`, dans 33 fichiers (composants dashboard/audit/invoice/layout/supplier + pages admin/invoice/supplier). Les 11 sites restants de l'inventaire correspondaient déjà aux 2 catégories de conversion (montant/date) sur des fichiers XAF déjà traités en (b), pas des sites distincts à convertir séparément. (d) **Sites intentionnellement laissés tels quels (9 call-sites, ne routent PAS vers le helper fr-FR figé)** car ils passent déjà une locale dynamique respectant la langue active de l'UI, ce que RT-4 ne reproche pas : `AdminRetentionPolicyPage.tsx:43`, `AdminRetentionDispositionPage.tsx:86`, `AdminArchiveCompliancePage.tsx:86`, `SecuritySettingsPage.tsx:271-272` (tous `toLocaleString(i18n.language)` / `toLocaleString(dateLocale)`) ; `PaymentsPage.tsx:85,356,438,441` (`toLocaleString(i18n.language === 'en' ? 'en-US' : 'fr-FR')` / `toLocaleDateString(...)` — déjà bilingue explicite) ; `AdminBackupsPage.tsx:92,173` (`toLocaleString('fr-FR', {...})` déjà explicite avec options). Forcer ces sites vers le helper fr-FR aurait cassé leur comportement bilingue existant en UI anglaise.
- **Tests :** nouveau `frontend/src/test/lib/format.test.ts` (TDD, RED confirmé avant création du helper — import `@/lib/format` inexistant faisait échouer la suite ; GREEN après, 9/9) : `formatAmount` groupe les milliers en notation fr-FR sans virgule US (assertion sur absence de `,` et présence de la séquence `900` groupée, PAS un espace ASCII littéral — `Intl` avec la locale `fr-FR` groupe avec U+202F, espace fine insécable, sous Node/ICU récent), accepte les chaînes numériques, retourne `'—'` pour null/undefined/vide/NaN, formate `0` en `'0'` (pas `'—'`) ; `formatDate` retourne `JJ/MM/AAAA` pour une chaîne ISO et un objet `Date`, `'—'` pour les valeurs invalides ; `formatDateTime` inclut la date au format FR et n'affiche jamais AM/PM. Suite complète frontend : **94/94** (85 baseline Task 9 + 9 nouveaux cas `format.test.ts`), 26 fichiers de test (25 + 1 nouveau). Aucune assertion existante ne portait sur le format US ou sur `XAF`, donc aucun test préexistant n'a eu besoin d'être modifié. `npx tsc --noEmit` : 0 erreur.
- **Règle préventive :** ne jamais appeler `toLocaleString`/`toLocaleDateString`/`toLocaleTimeString`/`Intl.*` sans argument de locale explicite — le défaut retombe sur la locale du runtime, jamais garantie française en production. Toujours passer par `frontend/src/lib/format.ts` (`formatAmount`/`formatDate`/`formatDateTime`, fr-FR par défaut) pour tout affichage de montant ou de date destiné à l'UI française par défaut ; pour un écran explicitement bilingue (respecte `i18n.language` en cours), passer la locale active explicitement plutôt que de forcer fr-FR. Avant d'introduire un nouveau littéral de devise, vérifier contre la devise fonctionnelle documentée du projet (XOF, Franc CFA BCEAO) plutôt que de copier un gabarit générique. ⚠ NOTE (corrigée par PROB-106, 2026-07-09) : cette affirmation est ERRONÉE — la devise du système est XAF (Franc CFA BEAC, zone CEMAC, Gabon), PAS XOF. Voir PROB-106.
- **Fichiers modifiés :** `frontend/src/lib/format.ts` (nouveau), `frontend/src/test/lib/format.test.ts` (nouveau), `frontend/src/components/dashboard/AgingBucketsWidget.tsx`, `frontend/src/components/reports/VolumeTrendSection.tsx`, `frontend/src/components/admin/IntegrationConnectors.tsx`, `frontend/src/components/audit/RetentionComplianceCard.tsx`, `frontend/src/components/invoice/InvoiceTimeline.tsx`, `frontend/src/components/layout/NotificationDropdown.tsx`, `frontend/src/components/supplier/SupplierRelationship.tsx`, `frontend/src/pages/DashboardPage.tsx`, `frontend/src/pages/ProfilePage.tsx`, `frontend/src/pages/PurchaseOrdersPage.tsx`, `frontend/src/pages/ReportsPage.tsx`, `frontend/src/pages/ApprovalQueuePage.tsx`, `frontend/src/pages/ArchivePage.tsx`, `frontend/src/pages/FinancialAuditPage.tsx`, `frontend/src/pages/GoodsReceiptsPage.tsx`, `frontend/src/pages/InvoiceCreatePage.tsx`, `frontend/src/pages/InvoiceDetailPage.tsx`, `frontend/src/pages/InvoiceListPage.tsx`, `frontend/src/pages/NotificationsPage.tsx`, `frontend/src/pages/admin/AdminAuditPage.tsx`, `frontend/src/pages/admin/AdminCompliancePage.tsx`, `frontend/src/pages/admin/AdminDelegationsPage.tsx`, `frontend/src/pages/admin/AdminMatchingConfigPage.tsx`, `frontend/src/pages/admin/IntegrationsPage.tsx`, `frontend/src/pages/admin/SupplierDetailPage.tsx`, `frontend/src/pages/admin/SuppliersPage.tsx`, `frontend/src/pages/supplier/SupplierDashboardPage.tsx`, `frontend/src/pages/supplier/SupplierDocumentsPage.tsx`, `frontend/src/pages/supplier/SupplierInvoicesPage.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`.

### [PROB-103] Chaînes JSX figées en français cassant le mode EN (RoleGuard/Header/Sidebar/DocumentUploader/ArchivePage/AdminBackupsPage/PaymentsPage/GoodsReceiptsPage/DashboardPage)
- **Catégorie :** Frontend / Internationalisation (i18n) / UX
- **Sévérité :** 🟡 Mineur (lot mineurs de l'audit ; `docs/AUDIT_GENERAL_2026-07-02.md` L.112 + L.263 — ~9 chaînes recensées, extension à un inventaire plus large une fois les fichiers ouverts).
- **Découvert :** 2026-07-04 — Audit correctif (Task 16). Plusieurs composants et pages affichaient du texte utilisateur écrit en dur en français dans le JSX au lieu de passer par `t()` : `RoleGuard.tsx` (message « Accès non autorisé »), `Sidebar.tsx` (`title="Système opérationnel"`), `Header.tsx` (`BREADCRUMB_MAP`, dict de labels FR figés défini au niveau module, plus le lien racine « Tableau de bord »), `DocumentUploader.tsx` (le hook `t` était importé mais **jamais utilisé** — message de dépassement de taille, textes de la zone de dépôt, indice de formats), `ArchivePage.tsx` (option « Tous les départements », bouton « Réinitialiser les filtres »), `AdminBackupsPage.tsx` (en-têtes de tableau + titre/texte vide de la carte d'historique), `PaymentsPage.tsx` (placeholder de note de paiement), `GoodsReceiptsPage.tsx` (placeholder « Qté »), et surtout `DashboardPage.tsx` où une vingtaine de sous-titres/labels (« Workflows de validation », « Suivi de vos factures en temps réel », « Volume traité par fournisseur (XOF) », etc.) restaient figés malgré l'utilisation de `t()` ailleurs dans le même fichier. Conséquence : basculer la langue sur EN via le sélecteur du `Header` laissait ces zones afficher du texte français figé.
- **Cause racine :** texte user-facing écrit directement dans le JSX au moment de l'implémentation initiale au lieu de passer systématiquement par `t()`, en particulier (1) quand un dict de labels (`BREADCRUMB_MAP`) est défini **au niveau module** — donc hors du corps du composant — un développeur ne peut pas y appeler `t()` directement et a été tenté d'y mettre des libellés bruts plutôt que des clés i18n résolues au rendu ; (2) `useTranslation()` importé par réflexe dans `DocumentUploader.tsx` mais le développeur a ensuite écrit les chaînes en dur sans jamais utiliser le `t` obtenu, et aucun lint/test ne détecte un hook importé-mais-sous-utilisé pour du texte figé ; (3) sur `DashboardPage.tsx`, la présence de nombreux appels `t()` corrects a donné une fausse impression de couverture complète, masquant les sous-titres restés en dur ajoutés au fil des itérations.
- **Solution appliquée :** ajout de clés fr/en dans `frontend/src/i18n/fr.json` / `en.json` et routage de chaque littéral via `t()` : nouveaux namespaces `roleGuard.*`, `sidebar.systemOperational`, `breadcrumb.*` (`newInvoice`, `approvalMatrix`, `securitySettings`, `detail`), `documentUploader.*` (`tooLarge` avec interpolation `{{name}}`/`{{max}}`, `dropHere`, `dragOrClick`, `formatsHint` avec `{{max}}`), plus des clés ajoutées aux namespaces existants `archive.*` (`allDepartments`, `resetFilters`), `admin.backups.*` (`colDate`, `colOperation`, `colStatus`, `colFile`, `colTriggeredBy`, `historyTitle`, `historyEmpty`), `payments.notesPlaceholder`, `grn.qtyPlaceholder` (le namespace réel du fichier `GoodsReceiptsPage.tsx` est `grn.*`, pas `goodsReceipts.*`), et une vingtaine de clés `dashboard.*` pour les sous-titres/labels résiduels. Le `BREADCRUMB_MAP` de `Header.tsx` stocke désormais des **clés i18n** (réutilisant `nav.*` existant pour `/dashboard`, `/invoices`, `/approvals`, `/purchase-orders`, `/payments`, `/goods-receipts`, `/reports`, `/financial-audit`, `/archive`, `/notifications`, `/profile`, `/admin/users`, `/admin/departments`, `/admin/audit`, `/admin/suppliers`, `/admin/integrations`) au lieu de libellés, résolues via `t(BREADCRUMB_MAP[pathname])` à l'intérieur de `useBreadcrumb()` (qui appelle maintenant `useTranslation()`), plutôt qu'au niveau module. 39 clés ajoutées de chaque côté (fr.json et en.json passent de 1094 à 1133 clés chacun, arbre resté symétrique).
- **Tests :** nouveaux `frontend/src/test/components/Header.test.tsx` (rendu avec `i18n.language='en'`, assertions que « Dashboard »/« Invoices »/« New invoice » apparaissent et que « Tableau de bord »/« Factures »/« Nouvelle facture » n'apparaissent plus) et `frontend/src/test/components/DocumentUploader.test.tsx` (rendu EN, assertion que « Drag and drop or click to add files » apparaît et que le texte FR figé n'apparaît plus) — TDD, RED confirmé avant le routage `t()` (les 3 assertions EN échouaient contre le code figé), GREEN après. Suite complète frontend : **99/99** (94 baseline Task 15 + 5 nouveaux cas), `npx tsc --noEmit` : 0 erreur.
- **Règle préventive :** tout texte user-facing DOIT passer par `t()` avec une clé fr+en symétrique ; ne jamais figer une chaîne française (ou anglaise) dans le JSX, même pour un `placeholder`, un `title` d'attribut, ou un sous-titre à côté d'un autre texte déjà correctement traduit. Un dict de labels défini au niveau module (type `BREADCRUMB_MAP`) doit stocker des **clés i18n**, jamais des libellés déjà traduits — la résolution via `t()` se fait uniquement à l'intérieur d'un composant ou d'un hook, jamais au niveau module. Avant d'ajouter une nouvelle clé, vérifier si un namespace existant (`nav.*`, `dashboard.*`, `admin.backups.*`, etc.) couvre déjà le même libellé plutôt que d'en dupliquer un ; vérifier aussi le nom réel du namespace utilisé par le fichier (ex. `grn.*` et non `goodsReceipts.*`) avant de suivre une convention supposée.
- **Fichiers modifiés :** `frontend/src/components/auth/RoleGuard.tsx`, `frontend/src/components/layout/Sidebar.tsx`, `frontend/src/components/layout/Header.tsx`, `frontend/src/components/invoice/DocumentUploader.tsx`, `frontend/src/pages/ArchivePage.tsx`, `frontend/src/pages/admin/AdminBackupsPage.tsx`, `frontend/src/pages/PaymentsPage.tsx`, `frontend/src/pages/GoodsReceiptsPage.tsx`, `frontend/src/pages/DashboardPage.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/test/components/Header.test.tsx` (nouveau), `frontend/src/test/components/DocumentUploader.test.tsx` (nouveau).

### [PROB-104] Noms de départements affichés en anglais en UI FR (RT-5, hardcode `nameEn`) + actions destructives sans confirmation (MAJEUR-F2) + bouton d'import PO trompeur (MAJEUR-F3)
- **Catégorie :** Frontend / Internationalisation (i18n) / UX / Confirmation d'actions destructives / Honnêteté fonctionnelle
- **Sévérité :** 🟠 Majeur (MAJEUR-F2 : neuf actions destructives — révocation de délégation, désactivation de compte, réinitialisation MFA, suppression de webhook, révocation de session, suppression d'élément de checklist/calendrier de conformité, suppression d'annonce, plus suppression de définition de rapport/connecteur/contrat détectées en balayage complémentaire — n'affichaient AUCUNE confirmation avant d'exécuter la mutation, contrairement aux quelques écrans qui utilisent déjà `window.confirm()`/`confirm()`). RT-5 et MAJEUR-F3 sont 🟡 mineurs, regroupés dans ce même correctif de lot car découverts et corrigés ensemble (Task 17).
- **Découvert :** 2026-07-04 — Audit correctif (Task 17). **RECADRAGE important après investigation** : le plan d'origine listait 4 volets, dont 2 se sont révélés être des prémisses fausses : (a) « 15 clés i18n EN manquantes côté backend » — **déjà résolu**, vérifié par `comm` sur `messages_fr.properties`/`messages_en.properties` : 244/244 clés, symétrique, 0 clé FR-only ; **rien à faire côté backend**. (b) « V45 rename départements » — **prémisse fausse, aucune migration nécessaire** : la table `departments` a **déjà** `name_fr` ET `name_en` (`V1__create_departments.sql` seed correctement les noms FR — « Direction des Ressources Humaines », « Informatique », etc.) et `DepartmentDTO` expose bien les deux champs. Le bug RT-5 réellement observé (« Information Technology (INFO) » affiché alors que l'UI est en FR) est un bug de **consommation frontend**, pas de schéma : 4 écrans affichaient `dept.nameEn` en dur au lieu de choisir selon `i18n.language` — `AdminDelegationsPage.tsx:149` (`{d.code} — {d.nameEn}`), `AdminUserFormPage.tsx:212` (`{dept.nameEn} ({dept.code})`), `InvoiceCreatePage.tsx:254` (`{d.nameEn} ({d.code})`), `SupplierInvoiceSubmitPage.tsx:204` (`{d.nameEn} ({d.code})`). `ProfilePage.tsx:263-265` faisait déjà le bon choix (`i18n.language==='fr' ? nameFr : nameEn`) et n'a pas été modifié. Les 2 volets restants étaient corrects : (c) MAJEUR-F2, neuf sites confirmés sans aucune confirmation (liste ci-dessus) — vérifiés un par un par lecture du code, plus 3 sites supplémentaires trouvés par un grep complémentaire de tous les `apiClient.delete` du frontend (`ReportBuilderPage.tsx:39` suppression de définition de rapport, `IntegrationConnectors.tsx:42` suppression de connecteur, `SupplierRelationship.tsx:41` suppression de contrat fournisseur) — non listés dans le plan d'origine mais couverts par la même règle. (d) MAJEUR-F3, `PurchaseOrdersPage.tsx` avait un `<input type=file>` déguisé en bouton « Import » dont le `onChange` définissait un message anglais figé mentant sur l'existence d'un import ERP en cours (« Bulk import via ERP integration is in progress… ») alors que l'intégration ERP est hors-scope (Module 12, cf. `docs/TASKS.md §B`).
- **Cause racine :** (RT-5) un développeur a utilisé `dept.nameEn` directement dans 4 écrans construits à des moments différents, sans repasser par le pattern déjà correct utilisé ailleurs (`ProfilePage.tsx`) — aucune fonction/helper partagé pour « choisir le nom de département selon la langue active » n'existait, donc chaque écran réinventait (ou oubliait de réinventer) le choix. (MAJEUR-F2) chaque action destructive a été câblée directement sur `mutate()` au moment de l'implémentation initiale du module correspondant, sans jamais introduire de composant de confirmation partagé — le projet n'avait qu'un pattern `window.confirm()` ad hoc utilisé de façon inconsistante sur 4 écrans seulement (`AdminChecklistTemplatesPage`, `EscalationRulesPage`, `PaymentAlertRulesPage`, `SuppliersPage`), jamais généralisé. (MAJEUR-F3) un message de statut a été écrit comme placeholder temporaire lors du développement initial de la page PO (avant que Module 12/ERP soit tranché hors-scope) et n'a jamais été retiré une fois la décision de scope actée.
- **Solution appliquée :** **(RT-5)** les 4 sites listés ci-dessus routent désormais `{i18n.language === 'fr' ? d.nameFr : d.nameEn}` (`useTranslation()` étendu à `const { t, i18n } = useTranslation()` là où seul `t` était déstructuré) ; `AdminDepartmentsPage.tsx:99` (colonne « autre langue » d'un tableau qui montre intentionnellement les deux noms côte à côte, appariée à la ligne 96) n'a **volontairement pas été touchée** — ce n'est pas un bug, c'est un tableau bilingue par design. **(MAJEUR-F2)** nouveau composant générique `frontend/src/components/ui/ConfirmDialog.tsx` (`open`/`title`/`message`/`confirmLabel`/`cancelLabel`/`onConfirm`/`onCancel`/`variant?: 'default'|'danger'`, `role="dialog"`, `aria-modal`, focus automatique sur le bouton de confirmation à l'ouverture, Escape → `onCancel`, rendu via `createPortal(document.body)`, style calqué sur la modale existante `EditUserModal` d'`AdminUsersPage.tsx`) câblé sur les 9 sites du plan d'origine plus les 3 sites supplémentaires détectés (12 sites au total) : `AdminDelegationsPage.tsx` (revoke), `MyDelegationsPage.tsx` (revoke), `AdminUsersPage.tsx` (toggle actif/inactif + reset MFA), `IntegrationsPage.tsx` (delete webhook), `SecuritySettingsPage.tsx` (revoke session), `AdminCompliancePage.tsx` (delete checklist item + delete calendar item), `AdminAnnouncementsPage.tsx` (delete announcement), `ReportBuilderPage.tsx` (delete report definition), `IntegrationConnectors.tsx` (delete connector), `SupplierRelationship.tsx` (delete contract). Chaque site introduit un state `«Xxx»TargetId`/`confirmAction` et ne déclenche la mutation que dans `onConfirm` du `ConfirmDialog` (`onSuccess` des mutations ferme aussi le dialogue). Les 4 sites qui utilisaient déjà `window.confirm()`/`confirm()` (`AdminChecklistTemplatesPage.tsx:226`, `EscalationRulesPage.tsx:147`, `PaymentAlertRulesPage.tsx:164`, `SuppliersPage.tsx:162`) et le site `ArchiveFolderTree.tsx:65` (également `window.confirm()`, découvert lors du grep complémentaire) ont été **laissés tels quels** — déjà fonctionnels, pas de régression, migration vers `ConfirmDialog` non prioritaire pour ce lot. **(MAJEUR-F3)** le faux handler `onChange` de l'`<input type=file>` et son message anglais figé ont été retirés ; le bouton « Import » est désormais un `<button disabled aria-disabled="true">` honnête avec `title`/note statique `t('po.importUnavailable')` = FR « Import ERP indisponible — saisie manuelle » / EN « ERP import unavailable — enter manually » ; la création manuelle de PO (`showCreate` + formulaire) reste pleinement fonctionnelle et inchangée.
- **Tests :** TDD — nouveau `frontend/src/test/components/ConfirmDialog.test.tsx` (7 cas : ne rend rien si `open=false` ; `role="dialog"` + titre/message affichés ; `onConfirm`/`onCancel` appelés au clic ; `onCancel` appelé sur `Escape` ; libellés `confirmLabel`/`cancelLabel` personnalisés ; classe `danger` appliquée au bouton de confirmation) — RED confirmé (import `@/components/ui/ConfirmDialog` inexistant faisait échouer la résolution du module), GREEN après création du composant (7/7). Nouveau `frontend/src/test/pages/InvoiceCreateDepartmentLocale.test.tsx` (2 cas, RT-5) : rendu réel avec `I18nextProvider`/`i18n` (pas un mock) sur `InvoiceCreatePage`, département mocké avec `nameFr="Informatique"`/`nameEn="Information Technology"` — `i18n.language='fr'` affiche l'option « Informatique (INFO) » et PAS « Information Technology » ; `i18n.language='en'` affiche l'option « Information Technology (INFO) » et PAS « Informatique ». RED confirmé en repassant temporairement (`git stash`) sur la version non corrigée d'`InvoiceCreatePage.tsx` : le test `i18n.language='fr'` échoue bien contre le hardcode `d.nameEn` (1 failed/2), GREEN après restauration du fix. Effet de bord détecté et corrigé sur un test préexistant : `InvoiceCreateDuplicateWarning.test.tsx` mockait entièrement `react-i18next` avec un stub `useTranslation()` ne fournissant que `t` (pas `i18n`), ce qui faisait planter `InvoiceCreatePage.tsx` (`Cannot read properties of undefined (reading 'language')`) une fois `i18n.language` introduit dans le composant — corrigé en ajoutant `i18n: { language: 'fr' }` au stub du mock (aucune assertion supprimée, comportement du mock étendu). Suite complète frontend : **108/108** (99 baseline Task 16 + 7 `ConfirmDialog.test.tsx` + 2 `InvoiceCreateDepartmentLocale.test.tsx`), `npx tsc --noEmit` : 0 erreur.
- **Règle préventive :** ne jamais afficher un champ bilingue (`nameFr`/`nameEn` ou équivalent) en dur sur une seule langue dans un composant — toujours choisir selon `i18n.language` (avec FR par défaut, langue principale du projet), jamais hardcoder l'une ou l'autre variante ; avant d'investiguer un bug de nom affiché dans la mauvaise langue, vérifier D'ABORD si les deux colonnes existent déjà en base (`\d table` / migration V1) avant de supposer qu'une migration de schéma est nécessaire — un bug d'affichage bilingue est très souvent un bug de consommation frontend, pas un gap de schéma. Toute action destructive (delete/revoke/disable/reset) DOIT demander une confirmation explicite à l'utilisateur avant d'exécuter la mutation — utiliser le composant partagé `ConfirmDialog` plutôt que d'improviser un `window.confirm()` ou pire, aucune confirmation ; lors d'un balayage pour ce type de correctif, grep systématiquement TOUS les appels `apiClient.delete` (et mutations PATCH destructrices) du frontend plutôt que de s'en tenir à la liste fournie initialement — des sites supplémentaires existent presque toujours. Ne jamais afficher à l'utilisateur un message qui affirme ou implique l'existence d'une fonctionnalité non implémentée (« import en cours », « bientôt disponible » avec une fausse temporalité) — préférer un état désactivé honnête avec un message clair sur l'indisponibilité et l'alternative (ici : saisie manuelle). Quand un test mocke entièrement `react-i18next`, s'assurer que le stub expose TOUS les champs du hook réellement utilisés par le composant testé (`t` ET `i18n` si le composant lit `i18n.language`), pas seulement ceux utilisés au moment où le mock a été écrit.
- **Fichiers modifiés :** `frontend/src/components/ui/ConfirmDialog.tsx` (nouveau), `frontend/src/pages/admin/AdminDelegationsPage.tsx`, `frontend/src/pages/MyDelegationsPage.tsx`, `frontend/src/pages/admin/AdminUsersPage.tsx`, `frontend/src/pages/admin/IntegrationsPage.tsx`, `frontend/src/pages/admin/SecuritySettingsPage.tsx`, `frontend/src/pages/admin/AdminCompliancePage.tsx`, `frontend/src/pages/admin/AdminAnnouncementsPage.tsx`, `frontend/src/pages/ReportBuilderPage.tsx`, `frontend/src/components/admin/IntegrationConnectors.tsx`, `frontend/src/components/supplier/SupplierRelationship.tsx`, `frontend/src/pages/admin/AdminUserFormPage.tsx`, `frontend/src/pages/InvoiceCreatePage.tsx`, `frontend/src/pages/supplier/SupplierInvoiceSubmitPage.tsx`, `frontend/src/pages/PurchaseOrdersPage.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/test/components/ConfirmDialog.test.tsx` (nouveau), `frontend/src/test/pages/InvoiceCreateDepartmentLocale.test.tsx` (nouveau), `frontend/src/test/pages/InvoiceCreateDuplicateWarning.test.tsx`. Aucune migration Flyway créée (confirmé : `name_fr`/`name_en` déjà en base depuis `V1`).

### [PROB-105] Libellé de révocation de session trompeur (MAJEUR-F1) + code mort (2ᵉ `useForm` fantôme + méthode service superseded) + `POST /ocr/extract` sans fichier → 500 au lieu de 400
- **Catégorie :** Frontend / Honnêteté d'UI · Code mort · API / Robustesse (validation d'entrée)
- **Sévérité :** 🟡 Mineur (trois nettoyages finaux du lot audit Track A, Task 18).
- **Découvert :** 2026-07-04 — Audit correctif (Task 18). **Prémisses vérifiées contre le code réel AVANT correction** (cf. Task 17 où 2 volets sur 4 étaient faux) : les trois se sont confirmées. (F1) `SecuritySettingsPage.tsx:285` — le bouton « Revoke »/« Révoquer » par ligne de session appelle `DELETE /admin/sessions/user/{userId}` ; côté backend `AdminSessionService.revokeUserSessions(userId)` → `sessionRepository.revokeAllForUser(...)` révoque **TOUTES** les sessions actives de l'utilisateur, pas la seule ligne affichée — le libellé et le dialogue de confirmation (« Révoquer cette session ? » au singulier) mentaient sur la portée. Aucun endpoint de révocation d'une session unique n'existe (vérifié : seul `revokeUserSessions`/`revokeAllForUser`), donc corriger le libellé est l'action minimale correcte. (2) Code mort : `InvoiceCreatePage.tsx:68` créait un **deuxième `useForm<DetailsFormData>()` jetable** (`const selectedSupplierId = useForm<DetailsFormData>().watch?.('supplierId')`) à chaque rendu, totalement déconnecté du vrai formulaire (ligne 82) — son `watch('supplierId')` valait donc toujours `undefined`, et la query `purchaseOrders` (`queryKey ['pos', …]`, `enabled: !!selectedSupplierId`) qu'il pilotait était toujours désactivée ET **consommée nulle part** (l'UI ligne 228 utilise `supplierPOs`, piloté par le vrai `watchedSupplierId`). `InvoiceDocumentService.generateDownloadUrl(UUID,UUID)` (2 arg) était superseded par `generateDownloadUrlAndLog(...)` (le contrôleur n'appelle QUE `…AndLog`) — la seule référence restante était un test (`InvoiceDocumentServiceTest.download_checksumMismatch_throwsValidation:243`) qui l'utilisait pour vérifier le rejet sur intégrité SHA-256 (comportement porté par `verifyStoredChecksum`, appelé par les DEUX méthodes). (3) `OcrController.extract` faisait `file.getBytes()` sans garde ; un part multipart vide (0 octet) passait au service → NPE/500 via le catch-all `Exception`→500 de `GlobalExceptionHandler` (un part **absent** était déjà 400 via `MissingServletRequestParameterException`, mais un part présent-mais-vide ne l'était pas).
- **Cause racine :** (F1) libellé écrit au singulier sans vérifier la portée réelle de l'endpoint sous-jacent (révocation globale). (2) le `useForm` fantôme de la ligne 68 est un vestige d'une première version du formulaire, jamais retiré quand le vrai `useForm` (ligne 82) + `supplierPOs` l'ont remplacé ; `generateDownloadUrl` est l'ancêtre de `generateDownloadUrlAndLog` (ajout de l'access-log P11-50), laissé en place après introduction du successeur. (3) validation d'entrée absente : on a supposé qu'un fichier serait toujours fourni et non vide, sans garde explicite pour le cas 0-octet.
- **Solution appliquée :** **(F1)** clés i18n `admin.security.revoke` / `revokeSessionConfirmTitle` / `revokeSessionConfirmBody` (fr+en, symétriques) reformulées pour refléter la portée « toutes les sessions » — FR « Déconnecter toutes les sessions » / « Déconnecter toutes les sessions de cet utilisateur ? » / corps explicitant « Toutes les sessions actives … révoquées … déconnecté partout » ; EN symétrique (« Log out all sessions » / « Log out all of this user's sessions? » / …) ; le fallback inline du bouton passe de `'Revoke'` à `'Log out all sessions'`. **(2)** suppression des lignes 68 + query `purchaseOrders` de `InvoiceCreatePage.tsx` (l'interface `PurchaseOrder` reste — utilisée par `supplierPOs`) ; suppression de `InvoiceDocumentService.generateDownloadUrl(UUID,UUID)` — l'assertion d'intégrité du test `download_checksumMismatch_throwsValidation` **retargetée** sur `generateDownloadUrlAndLog(...)` (même `verifyStoredChecksum`, plus l'assertion `verify(…never()).save()` prouvant qu'aucune entrée d'access-log n'est écrite sur mismatch) → aucune couverture perdue. **(3)** garde `if (file == null || file.isEmpty()) throw new ValidationException("error.ocr.empty_file")` dans `OcrController.extract` (→ 400 via `GlobalExceptionHandler`) ; nouvelle clé `error.ocr.empty_file` (messages_fr ISO-8859-1 pur-ASCII « Aucun fichier fourni pour l'extraction OCR. » / messages_en UTF-8 « No file provided for OCR extraction. »).
- **Tests :** nouveau `OcrControllerTest` (`@SpringBootTest @AutoConfigureMockMvc @ActiveProfiles("test")`, `@MockBean OcrService`) : `extract_EmptyFile_Returns400` (multipart `file` = `new byte[0]` → `status().isBadRequest()` + `$.success=false` + `verify(ocrService, never()).extract(...)` prouvant que le service n'est jamais atteint) et `extract_ValidFile_Returns200` (fichier PDF valide → 200 + `$.data.invoiceNumber`). Test service retargeté (voir ci-dessus). Gate : backend `./mvnw test` = **565/0/0** (563 baseline Task 14 +2 OCR ; le retarget ne change pas le compte, la méthode morte n'avait pas de test dédié en plus du mismatch retargeté) ; frontend `npx tsc --noEmit` 0, `npx vitest run` 109/109 (inchangé — pas de nouveau test frontend, F1 = valeurs i18n, code mort = suppression sans nouveau cas).
- **Règle préventive :** un libellé d'action DOIT décrire la portée RÉELLE de l'endpoint qu'il déclenche — avant de nommer un bouton « X cette session/cet élément », vérifier si le backend agit sur une seule entité ou sur toutes (ici `revokeAllForUser`) ; ne jamais laisser un `useForm`/hook instancié en double « pour lire une valeur » — un second `useForm()` non branché au `register` du formulaire réel ne partage pas l'état et vaut toujours sa valeur par défaut (piège silencieux : la query dépendante ne se déclenche jamais) ; avant de supprimer une méthode « morte » côté service, grep les TESTS aussi (pas seulement les callers de prod) — si un test l'utilise pour couvrir un comportement partagé, retarger l'assertion sur la méthode vivante plutôt que de jeter la couverture. Tout endpoint acceptant un `MultipartFile` DOIT garder explicitement `file == null || file.isEmpty()` → `ValidationException` (400) avant tout accès au contenu, plutôt que de laisser un NPE remonter en 500.
- **Fichiers modifiés :** `frontend/src/pages/admin/SecuritySettingsPage.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/pages/InvoiceCreatePage.tsx`, `src/main/java/com/oct/invoicesystem/domain/invoice/service/InvoiceDocumentService.java`, `src/test/java/com/oct/invoicesystem/domain/invoice/service/InvoiceDocumentServiceTest.java`, `src/main/java/com/oct/invoicesystem/domain/ocr/controller/OcrController.java`, `src/main/resources/i18n/messages_fr.properties`, `src/main/resources/i18n/messages_en.properties`, `src/test/java/com/oct/invoicesystem/domain/ocr/controller/OcrControllerTest.java` (nouveau). Aucune migration Flyway.

### [PROB-106] Devise du système : XAF (Franc CFA BEAC) inversé en XOF par PROB-102 — RÉGRESSION corrigée
- **Sévérité :** 🟡 Mineur (affichage de devise, aucune donnée corrompue — les montants sont stockés sans conversion) mais transversal (frontend entier).
- **Découvert :** 2026-07-09 — Revue de cohérence des rapports, confirmé par l'utilisateur (métier).
- **Cause racine :** PROB-102 (2026-07-04) a délibérément remplacé tous les littéraux `XAF` par `XOF` côté frontend, sur l'hypothèse ERRONÉE que la devise fonctionnelle était le Franc CFA BCEAO (XOF, Afrique de l'Ouest / UEMOA). **C'est géographiquement faux :** OCT (Owendo Container Terminal) est au **Gabon**, donc en zone **CEMAC / BEAC → Franc CFA d'Afrique centrale = XAF**. La note de PROB-102 se contredisait elle-même (« OCT est basé à Owendo/Gabon » puis conclut XOF). Le backend, lui, était resté correct (`Invoice.currency = "XAF"` par défaut, aucun XOF en Java/SQL) — seul le frontend avait été inversé à tort.
- **Solution appliquée :** remplacement de tous les `XOF` par `XAF` dans le frontend (38 occurrences, 11 fichiers) : composants (`AgingBucketsWidget`, `VolumeTrendSection`), pages (`DashboardPage`, `InvoiceCreatePage`, `ProfilePage`, `PurchaseOrdersPage`, `ReportsPage`, `SupplierInvoiceSubmitPage`), clés i18n (`fr.json`/`en.json` : `topSuppliersSubtitle`, `reports.trends.amount`, `reports.amountDue`, `admin.users.approvalLimit`), et le test `KpiBand.test.tsx`. Défauts de formulaire PO/facture ramenés à `'XAF'`, options de `<select>` de devise = XAF (Franc CFA/FCFA) + EUR + USD. Backend inchangé (déjà XAF).
- **Tests :** `npx tsc --noEmit` 0, `npx vitest run` vert (le seul test citant la devise, `KpiBand.test.tsx`, mis à jour XOF→XAF).
- **Règle préventive :** la devise fonctionnelle du système est **XAF (Franc CFA BEAC, zone CEMAC/Afrique centrale)** — OCT est au Gabon. Ne JAMAIS écrire `XOF` : XOF = BCEAO/UEMOA/Afrique de l'Ouest, hors périmètre. Avant d'introduire ou de « corriger » un littéral de devise, vérifier la zone monétaire du client (Gabon = CEMAC = XAF), pas un gabarit générique. La note préventive de PROB-102 (« devise fonctionnelle documentée = XOF ») est ERRONÉE et remplacée par celle-ci.
- **Fichiers modifiés :** `frontend/src/components/dashboard/AgingBucketsWidget.tsx`, `frontend/src/components/reports/VolumeTrendSection.tsx`, `frontend/src/pages/DashboardPage.tsx`, `frontend/src/pages/InvoiceCreatePage.tsx`, `frontend/src/pages/ProfilePage.tsx`, `frontend/src/pages/PurchaseOrdersPage.tsx`, `frontend/src/pages/ReportsPage.tsx`, `frontend/src/pages/supplier/SupplierInvoiceSubmitPage.tsx`, `frontend/src/i18n/fr.json`, `frontend/src/i18n/en.json`, `frontend/src/test/components/KpiBand.test.tsx`. Aucune migration Flyway.

### [PROB-107] Encodage de messages_fr.properties : bloc MFA UTF-16 en doublon + accents détruits — assaini

- **Sévérité :** 🟠 Majeur (i18n FR utilisateur-visible : paiements, alertes, webhooks, rapports, MFA) — aucune donnée métier corrompue, mais texte d'interface illisible.
- **Découvert :** 2026-07-11 — Inspection octet-par-octet de messages_fr.properties avant travaux UI/UX. Le `file` système le voyait « ASCII » car la majorité l'est ; la corruption était localisée et invisible sans hexdump.
- **Cause racine :** trois dégâts d'encodage cumulés, hérités de ré-encodages successifs du fichier par des outils qui normalisent l'encodage (Edit/Write re-décodent le fichier). (1) Un bloc MFA (15 clés) collé en **UTF-16LE** au milieu d'un fichier ASCII (1287 octets NUL), en **doublon** d'un bloc MFA canonique sain plus bas — en `.properties` Java la dernière occurrence gagne, donc le bloc UTF-16 était du code mort qui polluait le fichier et s'affichait en mojibake espacé. (2) 31 clés dont les accents avaient été écrasés en **U+FFFD** (« � » irrécupérable) par un aller-retour d'encodage antérieur. (3) 16 lignes supplémentaires en **double-encodage UTF-8** (`é` encodé UTF-8 puis relu Latin-1 → `Ã©`), plus 6 BOM `\uFEFF` parasites et un mojibake en commentaire.
- **Solution appliquée :** reconstruction complète du fichier via script Python **binaire** (jamais Edit/Write, qui corrompt ce fichier). Suppression du bloc MFA UTF-16 (détecté par présence d'octets NUL). Réparation des 31 clés U+FFFD via le fichier `messages_en.properties` (intact) comme référence 1:1 + texte FR d'origine. Décodage automatique Latin-1→UTF-8 des 16 lignes double-encodées. Réparation manuelle des 4 commentaires U+FFFD. Réécriture en **ASCII pur `\uXXXX` (majuscules)**, 0 octet NUL, 258 clés uniques (0 doublon). Vérifié : `./mvnw test` = 574/0/0, et rendu Java simulé des messages = accents corrects, 0 résidu.
- **Tests :** suite backend complète 574/574 (0 échec), aucun test cassé par le changement. Rendu runtime des clés réparées vérifié (Échec, Règle, créée, Âge, Intégration, double authentification — tous accents corrects).
- **Règle préventive :** les fichiers `.properties` i18n de ce projet sont en **ASCII avec échappement `\uXXXX`** (indépendant de l'encodage de lecture, donc robuste). **Ne JAMAIS éditer messages_fr.properties via un outil qui normalise l'encodage** (Edit/Write le re-décodent et corrompent les accents Latin-1 ou les fins de ligne) — utiliser un script binaire (`open(path, "rb"/"wb")`). Avant tout commit du fichier, vérifier `count(b"\x00") == 0` (aucun bloc UTF-16), `all(b < 128)` (ASCII pur), et l'absence de `Ã`/`�`. Cf. aussi la leçon de la branche feat/reports-pdf-metadata (Edit a corrompu 28 valeurs FR en U+FFFD).
- **Fichiers modifiés :** `src/main/resources/i18n/messages_fr.properties` (uniquement). Aucune migration Flyway. Branche `feat/i18n-fr-encoding`.

### [PROB-108] Regression SQLGrammarException sur InvoiceRepository.findAllWithFilters : CAST des parametres date null retires — restaure

- **Severite :** 🔴 Critique (500 sur tout rapport/liste passant par findAllWithFilters avec dates : cash-flow, compliance PDF, exports). Meme famille que PROB-054 / PROB-038.
- **Decouvert :** 2026-07-17 — Gate `./mvnw test` du travail PDF/export en cours : `CashFlowProjectionIntegrationTest.getCashFlowProjection_runsRealQuery_returnsOk` = 500 au lieu de 200 (test PostgreSQL reel, ecrit precisement pour prevenir PROB-054).
- **Cause racine :** dans le travail PDF/export en cours, la requete `findAllWithFilters` a ete revertee de `(CAST(:fromDate AS date) IS NULL OR ...)` vers `(:fromDate IS NULL OR ...)` (idem `toDate`). Sans le CAST explicite, PostgreSQL ne peut pas determiner le type du parametre au moment du prepare quand il est null (ou dans la branche `IS NULL`) -> `SQLGrammarException: could not determine data type of parameter` -> 500. C'est exactement le mode d'echec documente dans le commentaire au-dessus de la requete et couvert par `CashFlowProjectionIntegrationTest`.
- **Solution appliquee :** restauration des `CAST(:fromDate AS date)` / `CAST(:toDate AS date)` dans `InvoiceRepository.findAllWithFilters`. Aucun autre changement de logique.
...
### [PROB-114] N17/N18/N24 — i18n backend court-circuitee : messages metier en anglais brut + cles FR manquantes

- **Severite :** 🔴 Critique (i18n utilisateur-visible) — tout message metier multi-mots etait renvoye tel quel, identique en FR et EN, quel que soit `Accept-Language`.
- **Decouvert :** 2026-07-17 — findings N17 (confirme runtime : `POST /invoices/{id}/workflow/validate-n1` renvoyait le meme "Invoice is not in N1 validation state" en fr ET en), N18 (`error.access_denied` absente de messages_fr), N24 (`report.pdf.executive.title` absente de messages_fr).
- **Cause racine :** `GlobalExceptionHandler.resolve()` court-circuitait sur `message.contains(" ")` — heuristique "une cle n'a pas d'espace" qui renvoie VERBATIM tout message humain multi-mots. Comme les ~65 `throw new WorkflowException/ValidationException(...)` portaient du texte anglais (pas des cles), aucun n'etait traduisible. En prime, `handleValidationException`/`handleConstraintViolationException` concatenaient `getDefaultMessage()` sans `resolve()` (cles `validation.*` brutes).
- **Solution appliquee :**
  - **Racine** : `resolve()` reconnait desormais une cle par SA FORME (`^[A-Za-z0-9_]+(\.[A-Za-z0-9_]+)+$` — segments pointes, sans espace) au lieu de "pas d'espace" ; texte humain et cles inconnues restent inchanges.
  - **Workflow metier** : 17 messages du chemin BAP bascules sur des cles + valeurs FR/EN : `error.approval.*` (cannot_assign_n2_in_n1, not_in_n1_state, not_in_n2_state, not_ready_for_daf, no_authenticated_user, approver_is_submitter, n2_same_as_n1), `error.payment.*` (only_bon_a_payer, already_recorded), `error.invoice.*` (only_brouillon_rejete_updatable, only_brouillon_deletable, department_required, only_aa_can_create, supplier_name_required, supplier_email_required), `error.matching.*` (override_reason_too_short, only_mismatch_overridable).
  - **Bean-Validation (N23 partiel)** : `handleValidationException` et `handleConstraintViolationException` passent desormais chaque message de champ par `resolve()` ; le libelle global devient la cle `error.validation_failed` (FR/EN).
  - **N18/N24** : ajout des cles FR manquantes `error.access_denied` et `report.pdf.executive.title`.
  - Toutes les valeurs FR ajoutees en **pur-ASCII `\uXXXX`** via script Python binaire (lecon PROB-107 : ne jamais ecrire d'accent brut dans messages_fr).
- **Tests :** 5 assertions realignees sur les cles (niveau service : `PaymentServiceTest` x2, `ThreeWayMatchingServiceTest` x2) ou sur la valeur localisee (`PaymentControllerTest` x2 avec `Accept-Language: en` explicite — prouve la resolution par locale). Gate `./mvnw test` = **598/0/0**.
